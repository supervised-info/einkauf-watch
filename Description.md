...
Stand der nativen App: 2026-09-08 (Build 73, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
`TabView` **Einkauf | To-Do** auf iPhone (`EinkaufRoot`) und Watch (`WatchRoot`). Einkaufs-Tab bleibt `ContentView` / `WatchListView` ohne `TodoStore` in der Listen-UI. `SettingsSheet` (Einkauf **…** → **Einstellungen**) injiziert `TodoStore` für **To-Do Backup** (inkl. **Archiv teilen**) und `ShoppingStore` für **Einkauf Archiv**.
...
10. **Einkauf Archiv** — native-only. **Archiv teilen** exportiert `einkauf-archiv.json` (Share Sheet, Temp-Datei `yyyyMMdd_HHmm-einkauf-archiv.json`, analog Backup teilen). Kein Import, keine Archiv-Ansicht. Datei liegt im App-Group-Ordner `Einkauf/` neben `einkauf-local.json`.
11. **To-Do Backup** — native-only (nicht im HTML-Sheet). **Backup importieren…** / **Backup exportieren…** / **Backup teilen** für JSON `format: "todo-v3-json"` (`todo-liste.json`). **Archiv teilen** analog für `todo-archiv.json` (`yyyyMMdd_HHmm-todo-archiv.json`). `fileImporter` nur `.json`. Derselbe `TodoImport.offer` / `TodoStore.importAny`-Pfad wie To-Do-**…** und `onOpenURL`: leer → direkt setzen, sonst **Anhängen** / **Ersetzen**. Import hebt `revision` über den Peer (`max(lokal, import) + 1`) und synct. Einkauf-JSON (`kind: "einkauf-backup"`) wird mit deutschem Fehler abgelehnt, nie in `ShoppingStore` geschrieben. MD/CSV bleiben im To-Do-Overflow. To-Do-**…** Backup importieren bleibt. Archiv-JSON wird in dieser Version nicht importiert.
...
Native stellt **Darstellung** (Hell/Dunkel/System, Creme/Blau) davor. HTML hat **kein** Darstellung-Block im Sheet; Theme/Palette bleiben am Site-Mast. HTML schiebt **Alle Läden** JSON (`kind: "einkauf-laeden"`) zwischen Neuer Laden und Ladenweg — nur HTML. Native hängt **Einkauf Archiv** und **To-Do Backup** nach dem Wörterbuch an.
...
- [ ] Settings-Reihenfolge: Darstellung (Hell/Dunkel/System, Creme/Blau) → Aktueller Laden → Neuer Laden → Ladenweg (`sonstiges` movable, `vor`/`nach` locked) → Abteilungen hinzufügen → Reset → Stamm → Gespeicherte Listen (Apply + Swipe-Delete) → Wörterbuch → **Einkauf Archiv** (teilen, kein Import) → **To-Do Backup** (JSON import/export/teilen + Archiv teilen; Einkauf-JSON abgelehnt).
...
- [ ] `TabView` **Einkauf | To-Do** auf iPhone und Watch; getrennte Stores/Dateien/Backups (`todo-local.json` / `kind: "todo-local"` / `format: "todo-v3-json"`); Archive `einkauf-archiv.json` / `todo-archiv.json` (Append nur beim Löschen erledigter Einträge); WC-Context `{einkauf, todo}`; nie To-Do in `ShoppingStore` / `einkauf-local.json`.
- [ ] Archiv teilen nur in iPhone-Einstellungen (**Einkauf Archiv** und **To-Do Backup**), analog Backup-Share; kein Archiv im Listen-Overflow, kein Watch-Sync.
