...
Stand der nativen App: 2026-09-03 (Build 30, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
**Siri / App Intents** (iPhone-App `Einkauf` und Watch-App `EinkaufWatch`): App-Shortcut-**Utterance** muss **genau einmal** den App-Namen enthalten (`\(.applicationName)`). Apple erlaubt in Shortcut-Phrasen nur `AppEntity` und `AppEnum` — **kein** freies `String` (`items`) in der Phrase. Entdeckungs-Phrasen: „{App} Einkauf“, „Artikel zu {App} hinzufügen“, „Füge etwas zu {App} hinzu“ (`shortTitle` „Einkauf“, `cart.badge.plus`). Siri fragt danach nach **Artikel** (`requestValueDialog`). Antwort gesprochen z. B. „Milch, Butter und Eier“; getippt darf mit `Einkauf:` beginnen („Einkauf: Milch, Butter und Eier“). Parameter-Titel **Artikel**. Führendes `Einkauf:` / `Einkauf` im Parameter wird in `perform()` abgestreift (`SpeechItemSplitter.strippingTriggerPrefix`). Split wie `SpeechItemSplitter`: Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; trimmen; Mengenwörter bleiben am Namen („zwei Eier“). Jeder Teil derselbe Pfad wie getipptes Hinzufügen (`DepartmentGuesser` + `mappings`), ein Persist + WatchConnectivity (`ShoppingStore.addItems(fromSpeech:)`, `enableSync` true). `parameterSummary` darf `$items` zeigen (Intent-UI, nicht Utterance). Siri-Dialog: „1 Artikel hinzugefügt.“ / „3 Artikel hinzugefügt.“ / leer → „Keine Artikel erkannt.“ Intent lädt/speichert über `Persistence` / kurzlebigen `ShoppingStore`.
...
**Nur native / nicht ins HTML:** Watch (Geh-Modus, Titel gekürzter Laden + Einkauf xx/yy, Erledigte ausblendbar, WidgetKit-Complication `xx/yy`, kein Picker/Edit/Share, **kein** In-App-Mikro); iPhone-Geh-Modus mit eigenem Auge (Bearbeiten ungefiltert); **iPhone-Homescreen-Widget** (`systemSmall`/`systemMedium`, Tap öffnet Einkaufsliste); **Siri / App Intents** (Utterance mit App-Namen, danach Siri fragt nach Artikel; getippt optional `Einkauf:`); **PDF Liste teilen** mit leeren quadratischen Kästchen; Erscheinungsbild **System** (folgt iPhone-Appearance) plus Hell/Dunkel und Creme/Blau **nur in Einstellungen**, nicht in der Toolbar. TestFlight nicht erforderlich. Native-`guess` wertet Nutzer-Mappings vor Sonderregeln/Keywords aus (HTML-Reihenfolge kann abweichen).
...
- Kein Watch-in-App-Mikrofon, kein `Speech.framework`; Siri-Utterance mit App-Namen, danach Nachfrage **Artikel** (getippt optional `Einkauf:`)
...
- [ ] Siri / App Intents: Utterance mit genau einem App-Namen (kein `String`/`$items` in der Phrase); Siri fragt nach Artikel; getippte Antwort darf mit `Einkauf:` beginnen; Splitter wie Sprache, Guesser+Mappings, Persistenz+WatchConnectivity; leerer Text = freundliches No-op; kein Watch-Mikro, kein Speech.framework.
