...
Stand der nativen App: 2026-09-06 (Build 67, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
TestFlight ist nicht Voraussetzung. v1 ist nicht für den App-Store-Submit gedacht. To-Do (Phasen 1–10), iCloud-Inbox (Phasen 1–3), kompakter iPhone-Nav, optionale Artikel-Felder `imported`/`urgency` (Build 63, Chip-UX Build 64–65, `normal` = ↔ U+2194 Build 66, nur Einkauf) und das kombinierte iPhone-Homescreen-Widget Einkauf + To-Do (Build 67) sind **geliefert**. **Optional offen:** Inbox Phase 4 (concurrent Append). To-Do-Architektur: [`Docs/TodoIntegration.md`](Docs/TodoIntegration.md). Diese Datei beschreibt den gelieferten Stand (Build 67). Inbox-Arbeit nur unter **iCloud-Inbox (Zweitgerät)** — kein `Docs/InboxIntegration.md`. HTML-Site für die neuen Felder ist ein späteres PR.
...
| Family | Inhalt |
|---|---|
| `systemSmall` | Zwei Zeilen, kompakt: `Einkaufsliste: oo/xx/yy` (passt „Einkaufsliste“ nicht → Label **Einkauf**); `To Do (<Listenname>): oo/xx/yy`. Leere oder unbekannte Listen-ID → `To Do (Alle)`. Ein Tap (`widgetURL` `einkauf://list`). |
...
- [ ] iPhone-Widget (WidgetKit, iOS 17): `systemSmall` zwei Zeilen `Einkaufsliste: oo/xx/yy` (sonst **Einkauf**) und `To Do (<Liste>): oo/xx/yy` (`To Do (Alle)` bei leerer ID); `systemMedium`/`systemLarge` Mini-Tabelle Offen / Erledigt / Gesamt; To-Do nur aktuelle Liste; Taps `einkauf://list` / `einkauf://todo`; App Group `group.net.tschelle.einkauf`; nicht auf der Watch.
