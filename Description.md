...
Stand der nativen App: 2026-09-03 (Build 33, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
**Siri / App Intents** (iPhone-App `Einkauf` und Watch-App `EinkaufWatch`): App-Shortcut-**Utterance** muss **genau einmal** den App-Namen enthalten (`\(.applicationName)`). Apple erlaubt in Shortcut-Phrasen nur `AppEntity` und `AppEnum` — **kein** freies `String` (`items`) in der Phrase. Entdeckungs-Phrasen ausschließlich mit dem Trigger **besorgen** (nicht Bring-typisch „Artikel hinzufügen“, nicht „Einkauf“ allein als Cue): „{App} besorgen“, „Besorgen mit {App}“, „{App} zum Besorgen“ (`shortTitle` „Besorgen“, `cart.badge.plus`). Beispiel: Nutzer sagt „Hey Siri, Einkauf besorgen“ (App-Display-Name + besorgen) → Siri fragt, was besorgt werden soll (`requestValueDialog` „Was soll ich besorgen?“) → Antwort „Milch, Butter und Eier“. iOS kann das Wort **nicht** OS-weit reservieren; die markante Phrase ist Best Effort gegenüber Bring. Parameter-Titel **Artikel**. Führendes `Einkauf:` / `Einkauf` / `Besorgen:` / `Besorgen` im Parameter wird in `perform()` abgestreift (`SpeechItemSplitter.strippingTriggerPrefix`). Getippt darf die Dialog-Antwort weiter mit `Einkauf:` beginnen. Split wie `SpeechItemSplitter`: Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; trimmen; Mengenwörter bleiben am Namen („zwei Eier“). Jeder Teil derselbe Pfad wie getipptes Hinzufügen (`DepartmentGuesser` + `mappings`). **iPhone:** `ShoppingStore(enableSync: true)` + `addItems(fromSpeech:)` (Persist + WatchConnectivity); `openAppWhenRun = false`. Siri-Dialog: „1 Artikel hinzugefügt.“ / „3 Artikel hinzugefügt.“ / leer → „Keine Artikel erkannt.“ **Watch:** Intent konstruiert **keinen** `ShoppingStore` und schreibt **keinen** vollen `AppState` (`Persistence.write`). Nur Pending-Queue in der App Group `group.net.tschelle.einkauf` (`einkauf-siri-pending.json` neben `einkauf-local.json`); `openAppWhenRun = true`, damit die Watch-App in den Vordergrund kommt. Beim Aktivwerden drain't `consumeSiriPendingAdds()` die Queue: `addItems(fromSpeech:)` auf dem **live** Store, Persist+Sync, Complication-Reload. Dialog „Wird zur Liste hinzugefügt.“ / leer → „Alles klar.“ `perform()` wirft nicht; Fehler → Dialog „Speichern fehlgeschlagen.“ `parameterSummary` darf `$items` zeigen (Intent-UI, nicht Utterance).
