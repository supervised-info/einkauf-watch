...
- **`urgency`**: `"urgent"` | `"normal"` | `"later"`; fehlender oder unbekannter Key = `"normal"`. Nutzer wechselt per Chip (iPhone Geh+Edit, Watch Geh): `urgent` → `normal` → `later` → `urgent`. Icons: eilig ⚡, normal **leer** (keine sichtbare Glyphe, feste Min-Breite, kein `"  "`-Inhalt), später ↓ (U+2193). Encode schreibt das Feld immer.
...
- [ ] Einkauf-Artikel: `imported` nur Inbox-Abruf / expliziter Fremd-Datei-Anhang (nie Siri/Tippen/Stamm/Saved-List; Backup-Restore und eigenes PDF stempeln nicht); UI-Streifen teal/`theme.slate`, read-only. `urgency` Chip ⚡ / leer / ↓, Zyklus eilig→normal→später, iPhone und Watch. Nicht auf To-Do.
