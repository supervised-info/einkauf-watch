...
Stand der nativen App: 2026-09-06 (Build 64, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
TestFlight ist nicht Voraussetzung. v1 ist nicht für den App-Store-Submit gedacht. To-Do (Phasen 1–10), iCloud-Inbox (Phasen 1–3), kompakter iPhone-Nav und optionale Artikel-Felder `imported`/`urgency` (Build 63, Chip-UX Build 64, nur Einkauf) sind **geliefert**. **Optional offen:** Inbox Phase 4 (concurrent Append). To-Do-Architektur: [`Docs/TodoIntegration.md`](Docs/TodoIntegration.md). Diese Datei beschreibt den gelieferten Stand (Build 64). Inbox-Arbeit nur unter **iCloud-Inbox (Zweitgerät)** — kein `Docs/InboxIntegration.md`. HTML-Site für die neuen Felder ist ein späteres PR.
...
Je Artikel: Checkbox, Name (Tipp → Rename; leer/Abbrechen = keine Änderung), rechts trailing Import-Streifen (nur Anzeige) dann Dringlichkeits-Chip (tippbar, Zyklus wie Geh-Modus), Dept-`Picker` (alle `Department.allCases`), Ziehen, Swipe-Löschen.
...
- [ ] Einkauf-Artikel: `imported` nur Inbox-Abruf / expliziter Fremd-Datei-Anhang (nie Siri/Tippen/Stamm/Saved-List; Backup-Restore und eigenes PDF stempeln nicht); UI-Streifen teal/`theme.slate`, read-only, **rechts** vor dem Urgency-Chip (links nur Checkbox + Text). `urgency` Chip ⚡ / leerer umrandeter Chip / ↓ (nicht ○, nicht –), Zyklus eilig→normal→später, iPhone und Watch. Nicht auf To-Do.
