...
Stand der nativen App: 2026-09-03 (Build 23, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
| Watch-UI | `Sources/Watch` — `EinkaufWatchApp`, `WatchListView`, `WatchHoldToTalk`, `WatchSpeechRecognizer` (ObjC-Shim), `EinkaufWatch-Bridging-Header`, `WatchComplicationReload` |
...
| Xcode | 15+, iOS 17, watchOS 10, Sprache de, Marketing 1.0. Target **EinkaufWatch** linkt `Speech.framework` + `AVFoundation.framework` (`OTHER_LDFLAGS` `$(inherited) -framework Speech -framework AVFoundation`). Swift-`import Speech` löst auf der watchOS-Toolchain nicht auf (`No such module 'Speech'`), auch wenn das Framework gelinkt ist. Hold-to-Talk nutzt deshalb ein Watch-only Objective-C-Shim `WatchSpeechRecognizer` (`#import <Speech/Speech.h>` nur in der `.m`, nicht im Header), Bridging-Header `EinkaufWatch-Bridging-Header.h` via `SWIFT_OBJC_BRIDGING_HEADER` nur am Target EinkaufWatch. `SWIFT_ENABLE_EXPLICIT_MODULES = NO` bleibt dort. |
...
Nur Geh-Modus. Navigationsleiste **ausgeblendet** (`.toolbar(.hidden, for: .navigationBar)`): `.navigationTitle("")` reserviert die Bar weiter und lässt eine Lücke unter der Uhr. Die **Systemuhr** bleibt Status und sichtbar. Inhalt direkt darunter (`VStack(spacing: 0)`): zuerst die Augen-Leiste, darunter die Titelzeile **eine Zeile** (`Text(store.state.watchTitle)`): gekürzter Ladenname + zwei Leerzeichen + `Einkauf xx/yy` (`AppState.watchTitle`). Limit 6 Zeichen vor „Einkauf xx/yy“; länger: 5 Zeichen + `…`. Edeka/Aldi/Rewe/Lidl/dm ungekürzt; „Eigenes Layout“ → `Eigen…`. Zähler inkl. `vor`/`nach` (`doneCount/items.count`). **Nicht** `.navigationTitle(watchTitle)`.
...
Auge **zuerst über der Titelzeile**, links (`HStack { Button…; Spacer(); Hold-Mikro }`, Chrome über `watchTitle` und der `List` — **nicht** in `.topBarLeading`; **nicht** `.topBarTrailing`, die Uhr überdeckt das; **nicht** in der Toolbar, Navigationsleiste ausgeblendet): kleines SF-Symbol `eye` wenn Erledigte sichtbar, `eye.slash` wenn ausgeblendet — **plain Icon**, kein gefüllter runder watchOS-Button. Schrift `.font(.caption)` / ca. 14–16pt, `.imageScale(.small)`, `.buttonStyle(.plain)`. Glyph **grün** (`theme.good`, wie die erledigten Häkchen) wenn Erledigte sichtbar (`eye`); **grau** (`theme.muted`, wie Abteilungsüberschriften) wenn ausgeblendet (`eye.slash`). **Kompakte Zeile ~18–20pt**: **kein** `.frame(minHeight: 44)` / 44pt-Tapziel auf dem Image — das erzeugt leere Bänder zwischen Auge, Titel und erstem Listenartikel. Tap-Fläche eher horizontal über `contentShape`/Padding, ohne vertikale Totfläche. Darunter die Titelzeile kompakt, eine Zeile, lesbar (`theme.ink`). List: Top-`contentMargins` 0, erster Artikel direkt unter der Titelzeile; Leerzustände ohne allseitiges `.padding()`. Accessibility „Erledigte ausblenden“ / „Erledigte einblenden“. Tippen blendet abgehakte Artikel **nur in der Watch-Gehliste** aus; die Artikel bleiben auf der Liste und im Backup. Abteilungen ohne sichtbare Artikel verschwinden. Alles erledigt und ausgeblendet: kurze Zeile „Erledigte ausgeblendet.“ (Auge bleibt oben, Titel darunter). Flag nur auf der Watch (`UserDefaults` / `AppStorage` `einkauf.watch.hideCompleted`), **nicht** im einkauf-backup, **nicht** zum iPhone (das iPhone hat `einkauf.iphone.hideCompleted`). `watchTitle` und Complication zeigen weiter `xx/yy` der vollen Liste.

**Hold-Mikro:** in derselben Chrome-Zeile **trailing** nach `Spacer()`. SF-Symbol `mic` / `mic.fill` während der Aufnahme. `.font(.caption)`, `.imageScale(.small)`, `.buttonStyle(.plain)` — **kein** `.title2` / `.title`, **kein** Toolbar-Item (sonst der große gefüllte Kreis). Sprache **nur solange gedrückt** (press → Start, release → Stop und Commit). Kein Dauer-Zuhören. Erste Nutzung: System fragt Mikrofon und Spracherkennung; Watch-`Info.plist` `NSMicrophoneUsageDescription` und `NSSpeechRecognitionUsageDescription` (Deutsch, Zweck: Einkaufsliste per Sprache füllen). Entitlements bleiben App Group, kein extra Speech-Entitlement. Target `EinkaufWatch` linkt `Speech.framework` und `AVFoundation.framework`. Swift-Quellen importieren **kein** `Speech` (`No such module 'Speech'` trotz gelinktem Framework): `WatchHoldToTalkSession` ruft das ObjC-Shim `WatchSpeechRecognizer` (`Sources/Watch/WatchSpeechRecognizer.h/.m`) über `EinkaufWatch-Bridging-Header.h` (`SWIFT_OBJC_BRIDGING_HEADER` nur EinkaufWatch). Das Shim `#import <Speech/Speech.h>` + `#import <AVFoundation/AVFoundation.h>` nur in der `.m`; der Header bleibt Foundation-only, damit Swift das Speech-Modul nicht anfasst. On-device `SFSpeechRecognizer` + Audio-Engine im Shim, Locale `de_DE`, sonst Geräte-Locale. Permission beim ersten Druck. Release ohne nutzbaren Text: Erkennung abbrechen, Status „Nichts verstanden.“; verweigert: „Mikrofon nicht erlaubt.“ / „Spracherkennung nicht erlaubt.“ — kein Crash. Treffer: `SpeechItemSplitter` teilt an Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; Trim, Leere weg, Whitespace zusammenziehen; Mengenwörter bleiben im Namen („zwei Eier“), damit `DepartmentGuesser` / mappings greifen. Jedes Stück `addItem` über `addItems(fromSpeech:)` , Sync WatchConnectivity. Kurzes Haptic wenn Artikel dazukamen. **iPhone-Spracheingabe kommt später** (derselbe Splitter in `Sources/Shared`).
...
- [ ] Watch Hold-Mikro: gedrückt halten spricht, Loslassen splittet und `addItem`; Permissions in Watch-Info.plist; iPhone-Sprache später.
