...
Stand der nativen App: 2026-09-03 (Build 27, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
| Geteilter Code | `Sources/Shared` in beiden App-Targets (`ConnectivitySync.swift` nur Apple-Targets, nicht im SPM-Paket; `SpeechItemSplitter.swift` rein, Linux-testbar, später auch iPhone-Sprache). Widgets laden `einkauf-local.json` über `Persistence` (ohne `ShoppingStore` / WatchConnectivity im Widget-Prozess). |
...
| Watch-UI | `Sources/Watch` — `EinkaufWatchApp`, `WatchListView`, `WatchVoiceAdd`, `WatchComplicationReload` |
...
| Xcode | 15+, iOS 17, watchOS 10, Sprache de, Marketing 1.0. Watch-Spracheingabe **ohne** `Speech.framework` / `AVFoundation` / Bridging-Header: Mic öffnet ein **In-App-TextField**-Panel (Prompt „Artikel“, **Übernehmen** nimmt Fokus weg, wartet ~0,85s, speichert weich, schließt danach; X bricht ab) — **nicht** `TextFieldLink`, **nicht** `presentTextInputController`. Speech-Header sind in dieser watchOS-Toolchain nicht verfügbar (`Speech/Speech.h` file not found); kein ObjC-Shim, kein `import Speech`. |
...
Nur Geh-Modus. **Kein** `NavigationStack` (System-Texteingabe unter Stack + hidden toolbar beendet den Watch-Prozess). Kein `.toolbar(.hidden, for: .navigationBar)` als Host, kein `.containerBackground(..., for: .navigation)`. Plain `VStack`/`ZStack` mit Hintergrund `theme.paper`. `.navigationTitle("")` reserviert die Bar weiter und lässt eine Lücke unter der Uhr — weglassen. Die **Systemuhr** bleibt Status und sichtbar. Inhalt direkt darunter (`VStack(spacing: 0)`): zuerst die Augen-Leiste, darunter die Titelzeile **eine Zeile** (`Text(store.state.watchTitle)`): gekürzter Ladenname + zwei Leerzeichen + `Einkauf xx/yy` (`AppState.watchTitle`). Limit 6 Zeichen vor „Einkauf xx/yy“; länger: 5 Zeichen + `…`. Edeka/Aldi/Rewe/Lidl/dm ungekürzt; „Eigenes Layout“ → `Eigen…`. Zähler inkl. `vor`/`nach` (`doneCount/items.count`). **Nicht** `.navigationTitle(watchTitle)`.
...
Auge **zuerst über der Titelzeile**, links (`HStack { Button…; Spacer(); Mikro }`, Chrome über `watchTitle` und der `List` — **nicht** in `.topBarLeading`; **nicht** `.topBarTrailing`, die Uhr überdeckt das; **nicht** in der Toolbar, Navigationsleiste ausgeblendet): kleines SF-Symbol `eye` wenn Erledigte sichtbar, `eye.slash` wenn ausgeblendet — **plain Icon**, kein gefüllter runder watchOS-Button. Schrift `.font(.caption)` / ca. 14–16pt, `.imageScale(.small)`, `.buttonStyle(.plain)`. Glyph **grün** (`theme.good`, wie die erledigten Häkchen) wenn Erledigte sichtbar (`eye`); **grau** (`theme.muted`, wie Abteilungsüberschriften) wenn ausgeblendet (`eye.slash`). **Kompakte Zeile ~18–20pt**: **kein** `.frame(minHeight: 44)` / 44pt-Tapziel auf dem Image — das erzeugt leere Bänder zwischen Auge, Titel und erstem Listenartikel. Tap-Fläche eher horizontal über `contentShape`/Padding, ohne vertikale Totfläche. Darunter die Titelzeile kompakt, eine Zeile, lesbar (`theme.ink`). List: Top-`contentMargins` 0, erster Artikel direkt unter der Titelzeile; Leerzustände ohne allseitiges `.padding()`. Accessibility „Erledigte ausblenden“ / „Erledigte einblenden“. Tippen blendet abgehakte Artikel **nur in der Watch-Gehliste** aus; die Artikel bleiben auf der Liste und im Backup. Abteilungen ohne sichtbare Artikel verschwinden. Alles erledigt und ausgeblendet: kurze Zeile „Erledigte ausgeblendet.“ (Auge bleibt oben, Titel darunter). Flag nur auf der Watch (`UserDefaults` / `AppStorage` `einkauf.watch.hideCompleted`), **nicht** im einkauf-backup, **nicht** zum iPhone (das iPhone hat `einkauf.iphone.hideCompleted`). `watchTitle` und Complication zeigen weiter `xx/yy` der vollen Liste.

**Mikro:** in derselben Chrome-Zeile **trailing** nach `Spacer()`. SF-Symbol `mic` (`mic.fill` / `theme.oxide` während das Diktier-Panel offen ist). `.font(.caption)`, `.imageScale(.small)`, `.buttonStyle(.plain)` — **kein** `.title2` / `.title`, **kein** Toolbar-Item (sonst der große gefüllte Kreis). **Tippen** setzt `showDictate` und öffnet ein **In-App-TextField**-Panel (restliche Fläche unter Chrome/Titel, kein System-Modal): Prompt „Artikel“ (watchOS-`TextField` hat Scribble/Diktat über die Systemsteuerung). **Kein Auto-Focus** beim Öffnen — Nutzer tippt das Feld einmal. Primärknopf **Übernehmen**: zuerst `focused = false` (Keyboard/Diktat zu), Status „…“, ~0,85s warten auf MainActor, dann Store mutieren, **erst danach** `showDictate = false`. **X** / Abbrechen schließt ohne Commit (gesperrt während des Wartens). Commit erst nach **Übernehmen** auf unserer UI — nicht während Keyboard-/Diktat-Dismiss, **nicht** `onSubmit`/`TextFieldLink`, **nicht** `presentTextInputController` auf einem `WKInterfaceController` (Crash-on-Dismiss in reiner SwiftUI-Watch-App), **nicht** Hold-to-Talk, nicht gedrückt halten, kein Dauer-Zuhören, kein `Speech.framework`. Speech-Header sind in dieser watchOS-Toolchain nicht verfügbar (`Speech/Speech.h` file not found); kein ObjC-Shim, kein Bridging-Header, kein `import Speech`, kein `SWIFT_ENABLE_EXPLICIT_MODULES`-Workaround. Kein `NSMicrophoneUsageDescription` / `NSSpeechRecognitionUsageDescription` — In-App-`TextField` braucht diese Keys nicht. Entitlements bleiben App Group. Abbruch: kein Crash, kein Persist. Leerer oder unbrauchbarer Text: Status „Nichts verstanden.“ Treffer: `SpeechItemSplitter` teilt an Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; Trim, Leere weg, Whitespace zusammenziehen; Mengenwörter bleiben im Namen („zwei Eier“), damit `DepartmentGuesser` / mappings greifen. `addItemsFromWatchVoice(_:)` legt alle Stücke in **einem** State-Update an und schreibt **nur** `Persistence.save`/`write` — **kein** `WatchComplicationReload.timelines()` und **kein** `sync?.broadcast` im Commit. Nach ~2s einmal broadcast + Complication-Reload (Fehler schlucken). Toggle/andere Pfade behalten `persistQuietly` inkl. WidgetKit. Kein Haptic auf diesem Pfad. Status in der Listen-Chrome: „…“ vor dem Warten, dann „N Artikel.“ / „Nichts verstanden.“ / „Speichern fehlgeschlagen.“ **iPhone-Spracheingabe kommt später** (`addItems(fromSpeech:)` + derselbe Splitter in `Sources/Shared`).
...
**Nur native / nicht ins HTML:** Watch (Geh-Modus, Titel gekürzter Laden + Einkauf xx/yy, Erledigte ausblendbar, **In-App-TextField**-Diktat Mehrfach-Split, WidgetKit-Complication `xx/yy`, kein Picker/Edit/Share); iPhone-Geh-Modus mit eigenem Auge (Bearbeiten ungefiltert); **iPhone-Homescreen-Widget** (`systemSmall`/`systemMedium`, Tap öffnet Einkaufsliste); **PDF Liste teilen** mit leeren quadratischen Kästchen; Erscheinungsbild **System** (folgt iPhone-Appearance) plus Hell/Dunkel und Creme/Blau **nur in Einstellungen**, nicht in der Toolbar. TestFlight nicht erforderlich. Native-`guess` wertet Nutzer-Mappings vor Sonderregeln/Keywords aus (HTML-Reihenfolge kann abweichen).
...
- [ ] Watch-Mikro: Tippen öffnet In-App-TextField-Panel (kein Auto-Focus); Übernehmen unfocused, wartet ~0,85s, `addItemsFromWatchVoice` weich speichert (kein WidgetKit/Sync im Commit, beides nach ~2s); kein Speech-Framework, kein TextFieldLink/presentTextInput; iPhone-Sprache später.
