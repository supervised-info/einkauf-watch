...
Stand der nativen App: 2026-09-03 (Build 29, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
**Siri / App Intents** (iPhone-App `Einkauf` und Watch-App `EinkaufWatch`): Phrase beginnt mit **Einkauf** (optional Doppelpunkt), danach Freitext der Artikel. Gesprochen z. B. „Einkauf Milch, Butter und Eier“, getippt „Einkauf: Milch, Butter und Eier“. Zusätzliche Entdeckungs-Phrasen mit App-Namen (`applicationName`). Parameter-Titel **Artikel**. Führendes `Einkauf:` / `Einkauf` im Parameter wird abgestreift, falls Siri es nochmal mitliefert. Split wie `SpeechItemSplitter`: Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; trimmen; Mengenwörter bleiben am Namen („zwei Eier“). Jeder Teil derselbe Pfad wie getipptes Hinzufügen (`DepartmentGuesser` + `mappings`), ein Persist + WatchConnectivity (`ShoppingStore.addItems(fromSpeech:)`, `enableSync` true). Siri-Dialog: „1 Artikel hinzugefügt.“ / „3 Artikel hinzugefügt.“ / leer → „Keine Artikel erkannt.“ Intent lädt/speichert über `Persistence` / kurzlebigen `ShoppingStore`.
...
**Nur native / nicht ins HTML:** Watch (Geh-Modus, Titel gekürzter Laden + Einkauf xx/yy, Erledigte ausblendbar, WidgetKit-Complication `xx/yy`, kein Picker/Edit/Share, **kein** In-App-Mikro); iPhone-Geh-Modus mit eigenem Auge (Bearbeiten ungefiltert); **iPhone-Homescreen-Widget** (`systemSmall`/`systemMedium`, Tap öffnet Einkaufsliste); **Siri / App Intents** „Einkauf …“ / „Einkauf: …“; **PDF Liste teilen** mit leeren quadratischen Kästchen; Erscheinungsbild **System** (folgt iPhone-Appearance) plus Hell/Dunkel und Creme/Blau **nur in Einstellungen**, nicht in der Toolbar. TestFlight nicht erforderlich. Native-`guess` wertet Nutzer-Mappings vor Sonderregeln/Keywords aus (HTML-Reihenfolge kann abweichen).
...
- Kein Watch-in-App-Mikrofon, kein `Speech.framework`; Artikel per Siri-Phrase **„Einkauf …“** / **„Einkauf: …“**
...
- [ ] Siri / App Intents: „Einkauf …“ / „Einkauf: …“ fügt Artikel hinzu (Splitter wie Sprache, Guesser+Mappings, Persistenz+WatchConnectivity); leerer Text = freundliches No-op; kein Watch-Mikro, kein Speech.framework.
