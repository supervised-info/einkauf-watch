...
- **`urgency`**: `"urgent"` | `"normal"` | `"later"`; fehlender oder unbekannter Key = `"normal"`. Nutzer wechselt per Chip **nur iPhone** (Geh+Edit): `urgent` → `normal` → `later` → `urgent`. Watch zeigt denselben Chip nur an (kein Tap, kein `cycleItemUrgency`). Icons: eilig ⚡, normal **leerer umrandeter Chip** (nur Rahmen, keine Glyphe innen, nicht ○ und nicht –, **kompakt dieselbe Min-Breite/-Höhe wie ⚡/↓**, keine breite Pille; auf dem iPhone tippbar; kein `"  "`-Inhalt), später ↓ (U+2193). Encode schreibt das Feld immer.
