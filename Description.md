...
Stand der nativen App: 2026-09-03 (Build 24, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
| Xcode | 15+, iOS 17, watchOS 10, Sprache de, Marketing 1.0. Watch-Spracheingabe **ohne** `Speech.framework` / `AVFoundation` / Bridging-Header: Mic öffnet das WatchKit-System-Diktat (`presentTextInputController`). Speech-Header sind in dieser watchOS-Toolchain nicht verfügbar (`Speech/Speech.h` file not found); kein ObjC-Shim, kein `import Speech`. |
...
**Mikro:** in derselben Chrome-Zeile **trailing** nach `Spacer()`. SF-Symbol `mic` / `mic.fill` während das Diktatblatt offen ist. `.font(.caption)`, `.imageScale(.small)`, `.buttonStyle(.plain)` — **kein** `.title2` / `.title`, **kein** Toolbar-Item (sonst der große gefüllte Kreis). **Tippen** öffnet das WatchKit-System-Diktat (`WKApplication.shared().visibleInterfaceController` bzw. `rootInterfaceController` → `presentTextInputController(withSuggestions:allowedInputMode:completion:)`, `WKTextInputMode.plain`) — **nicht** Hold-to-Talk, nicht gedrückt halten, kein Dauer-Zuhören, kein `Speech.framework`. Speech-Header sind in dieser watchOS-Toolchain nicht verfügbar (`Speech/Speech.h` file not found); kein ObjC-Shim, kein Bridging-Header, kein `import Speech`, kein `SWIFT_ENABLE_EXPLICIT_MODULES`-Workaround. Kein `NSMicrophoneUsageDescription` / `NSSpeechRecognitionUsageDescription` — System-Diktat über `presentTextInputController` braucht diese Keys nicht. Entitlements bleiben App Group. Abbruch (nil): kein Status. Leerer oder unbrauchbarer Text: Status „Nichts verstanden.“ Treffer: `SpeechItemSplitter` teilt an Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; Trim, Leere weg, Whitespace zusammenziehen; Mengenwörter bleiben im Namen („zwei Eier“), damit `DepartmentGuesser` / mappings greifen. Mehrere Diktat-Phrasen werden mit Komma verbunden, dann gesplittet. Jedes Stück `addItem` über `addItems(fromSpeech:)`, Sync WatchConnectivity. Kurzes Haptic wenn Artikel dazukamen. **iPhone-Spracheingabe kommt später** (derselbe Splitter in `Sources/Shared`).
...
- [ ] Watch-Mikro: Tippen öffnet System-Diktat (`presentTextInputController`), Splitter/`addItems(fromSpeech:)`; kein Speech-Framework; iPhone-Sprache später.
