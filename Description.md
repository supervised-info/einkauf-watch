...
TestFlight ist nicht Voraussetzung. v1 ist nicht für den App-Store-Submit gedacht. To-Do (Phasen 1–10), iCloud-Inbox (Phasen 1–3), kompakter iPhone-Nav und optionale Artikel-Felder `imported`/`urgency` (Build 63, Chip-UX Build 64–65, `normal` = ↔ U+2194 Build 66, nur Einkauf) sind **geliefert**. Watch-Listen: oben `watchTopSafeChrome()` (Build 67), Liste darunter weiter kompakt. **Optional offen:** Inbox Phase 4 (concurrent Append). To-Do-Architektur: [`Docs/TodoIntegration.md`](Docs/TodoIntegration.md). Diese Datei beschreibt den gelieferten Stand (Build 67). Inbox-Arbeit nur unter **iCloud-Inbox (Zweitgerät)** — kein `Docs/InboxIntegration.md`. HTML-Site für die neuen Felder ist ein späteres PR.

## To-Do

Geliefert (Build 62). Begleit-Slice zur HTML-PWA [todo](https://supervised-info.github.io/todo/) (Pages `todo/Description_index.md`). Native scrapt die Website nicht. Brücke: JSON `format: "todo-v3-json"` (`todo-liste.json`) plus MD/CSV (`TodoMarkdown` / `TodoCSV`, Dateien `todo-liste.md` / `todo-liste.csv`, **volle Liste** unabhängig vom Auge und vom Listenfilter). Kein Live-localStorage, kein Netz. **Kein** To-Do-Homescreen-Widget. Benannte Listen sind nativ gelandet; **kein** HTML-Parity für Site-Mast — HTML-Listen sind ein separates Follow-up auf derselben JSON-Schema-Erweiterung. Reopen, Sort, Suche, MD/CSV, Listen, **Einstellungen → To-Do Backup** (JSON) und `#uid` Badge + reopen-Pills wie HTML sind gelandet. iPhone-Listen ohne großen Nav-Titel, Toolbar kompakt (`einkaufToolbarChrome`).
...
Navigationsleiste **ausgeblendet** (`.toolbar(.hidden, for: .navigationBar)`): `.navigationTitle("")` reserviert die Bar weiter und lässt eine Lücke unter der Uhr. Die **Systemuhr** bleibt Status und sichtbar. `VStack(spacing: 0)` mit `watchTopSafeChrome()`: `.safeAreaPadding(.top)` plus kleines Extra-Padding (~16pt), damit Auge und `watchTitle` Gehäuserand / Uhr räumen — **ohne** großen Leerraum zwischen Auge, Titel und erstem Listenartikel und **ohne** zweiten `safeAreaInset`-Zähler-Header. Darin zuerst die Augen-Leiste, darunter die Titelzeile **eine Zeile** (`Text(store.state.watchTitle)`): gekürzter Ladenname + zwei Leerzeichen + `Einkauf oo/xx/yy` (`AppState.watchTitle`). Limit 6 Zeichen vor „Einkauf oo/xx/yy“; länger: 5 Zeichen + `…`. Edeka/Aldi/Rewe/Lidl/dm ungekürzt; „Eigenes Layout“ → `Eigen…`. Zähler inkl. `vor`/`nach` (`openCount/doneCount/items.count`). **Nicht** `.navigationTitle(watchTitle)`. Gleicher oberer Chrome auf der To-Do-Watch-Liste.
