...
**Ablauf (Phase 2+3 geliefert, Abruf-Auswahl Build 60, Löschen Build 61, Phase 4 folgt):**
...
Wann / wo / wie — nur die Schritte, die **du** machst. **Inbox verbinden…** ist in der App (Build 59). **Inbox abrufen** zeigt eine Auswahl (Build 60) und **Löschen** pro Zeile ohne Import (Build 61). Die zwei Kurzbefehle baust **du** einmal auf dem Zweit-iPhone nach dem Rezept **Kurzbefehl (Phase 3)**.
...
4. **Inbox verbinden…** **Wann:** jetzt (Phase 2, Build 59). **Wo:** Einkauf-App auf dem **Haupt-iPhone**, Overflow **…** (nicht To-Do). **Wie:** einmal **Inbox verbinden…** → Dateien-Picker auf genau diese `inbox.txt` (`.plainText` / `public.text` / `.txt`, Security-scoped Bookmark). Nicht To-Do-Dateien, nicht ein Backup-JSON. Danach **Inbox abrufen** (Build 60: Auswahl; Build 61: **Löschen** ohne Import).
...
Ohne Schritt 1–3 funktioniert nichts. Schritt 4 ist mit Build 59 verfügbar; die Auswahl beim Abrufen mit Build 60; **Löschen** im Sheet mit Build 61. Schritt 5 ist das Rezept in dieser Datei.
...
- Phase 2: App-UI (**Inbox verbinden…**, Bookmark) — geliefert Build 59. **Inbox abrufen** mit Auswahl (`InboxRetrieveSheet`, Abgewählte bleiben in der Datei) — Build 60. **Löschen** pro Zeile ohne Import (sofortiges Rewrite, Selection nachrücken) — Build 61. Parser-Grundtests (leer, Kommentare, Items, BOM) plus Partition ausgewählt/Rest plus Entfernen/Selection-Shift.
...
- [ ] iCloud-Inbox Phase 2 (Build 59) + Abruf-Auswahl (Build 60) + **Löschen** (Build 61): Einkauf-**…** **Inbox verbinden…** / **Inbox abrufen** (Sheet, alle markiert, **Übernehmen**, **Löschen** pro Zeile ohne Import); ausgewählte `inbox.txt`-Zeilen → `ShoppingStore.addItems(fromSpeech:)`; Abgewählte bleiben in der Datei; gelöschte nie in `ShoppingStore`; nie To-Do; kein CloudKit / kein iCloud-Entitlement.
