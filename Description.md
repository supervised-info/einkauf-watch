...
Je Artikel: Checkbox, dann **zwei Zeilen** — oben Name (Tipp → Rename; leer/Abbrechen = keine Änderung; volle verfügbare Textbreite, nicht neben dem Picker), unten Dept-`Picker` (`DepartmentCatalog.knownIds`: Builtins plus `customDepartments`; volle Breite, **kein** `.fixedSize`, damit lange Titel wie „Drogerie & Haushalt“ den Namen nicht in 2–3 Zeichen pro Zeile quetschen). Daneben bleiben Dringlichkeits-Chip (tippbar, Zyklus wie Geh-Modus) und ganz rechts Import-Streifen (nur Anzeige, äußerster Trailing-Slot), plus List-Löschen und Ziehen. **Geh-Modus** bleibt eine Zeile (Checkbox + Name + Chip + Import, kein Picker).
...
- [ ] iPhone-Edit-Zeile zweizeilig: Name volle Breite oben, Abteilungs-Picker volle Breite darunter (kein `.fixedSize` am Picker); Löschen/Checkbox/Ziehen/Chip/Import bleiben; Geh-Modus unverändert eine Zeile.
