...
Stand der nativen App: 2026-09-11 (Build 80, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
TestFlight ist nicht Voraussetzung. v1 ist nicht für den App-Store-Submit gedacht. To-Do (Phasen 1–10), iCloud-Inbox (Phasen 1–3), kompakter iPhone-Nav, optionale Artikel-Felder `imported`/`urgency` (Build 63, Chip-UX Build 64–65, `normal` = ↔ U+2194 Build 66, nur Einkauf), das kombinierte iPhone-Homescreen-Widget Einkauf + To-Do (Build 67), die vereinheitlichte Small-Widget-Typografie (Build 68), die gestapelten Small-Blöcke (Build 69), der Widget-Reload-Debounce / Hang-Fix (Build 70), die Watch-Complication-Titel **Einkauf** / aktueller To-Do-Listenname (Build 71), das Archiv gelöschter erledigter Einträge (Build 72, `einkauf-archiv.json` / `todo-archiv.json`), der Inbox-Abruf mit iCloud-Download vor dem Lesen (Build 73), der iPhone-Tab **Einstellungen** (Build 74), das Einzel-Löschen von Archiv-Einträgen in Einstellungen (Build 75), **Zurückspielen** aus dem Archiv (Build 76), die manuelle Stamm-Reihenfolge in Einstellungen (Build 77), Abhaken nur am Kreis (iPhone-Geh-Modus und To-Do, Build 78), das Bearbeiten gespeicherter Einkaufs-Listen in Einstellungen (Build 79) und **eigene Abteilungen** (anlegen / umbenennen / löschen, Build 80) sind **geliefert**. **Optional offen:** Inbox Phase 4 (concurrent Append); HTML-UI für eigene Abteilungen. To-Do-Architektur: [`Docs/TodoIntegration.md`](Docs/TodoIntegration.md). Diese Datei beschreibt den gelieferten Stand (Build 80). Inbox-Arbeit nur unter **iCloud-Inbox (Zweitgerät)** — kein `Docs/InboxIntegration.md`. HTML-Site für die neuen Felder ist ein späteres PR.

## To-Do

Geliefert (Build 62). Begleit-Slice zur HTML-PWA [todo](https://supervised-info.github.io/todo/) (Pages `todo/Description_index.md`). Native scrapt die Website nicht. Brücke: JSON `format: "todo-v3-json"` (`todo-liste.json`) plus MD/CSV (`TodoMarkdown` / `TodoCSV`, Dateien `todo-liste.md` / `todo-liste.csv`, **volle Liste** unabhängig vom Auge und vom Listenfilter). Kein Live-localStorage, kein Netz. iPhone-Homescreen-Widget kombiniert Einkauf + To-Do der aktuellen Liste (kein separates To-Do-only-Widget, kein per-Liste-Raster). Benannte Listen sind nativ gelandet; **kein** HTML-Parity für Site-Mast — HTML-Listen sind ein separates Follow-up auf derselben JSON-Schema-Erweiterung. Reopen, Sort, Suche, MD/CSV, Listen, **Einstellungen → To-Do Backup** (JSON) und `#uid` Badge + reopen-Pills wie HTML sind gelandet. iPhone-Listen ohne großen Nav-Titel, Toolbar kompakt (`einkaufToolbarChrome`).

### Isolation (nicht mit Einkauf mischen)

`TabView` **Einkauf | To-Do | Einstellungen** auf dem iPhone (`EinkaufRoot`, dritter Tab `gearshape`); Watch (`WatchRoot`) bleibt **Einkauf | To-Do**. Einkaufs-Tab bleibt `ContentView` / `WatchListView` ohne `TodoStore` in der Listen-UI. `SettingsSheet` ist der iPhone-Tab **Einstellungen** (kein Sheet aus den Listen-**…**): injiziert `TodoStore` für **To-Do** (JSON-Backup inkl. **Archiv…** / **Archiv teilen**) und `ShoppingStore` für **Einkauf** (Backup, **Archiv…** / **Archiv teilen**, Inbox, Laden/Stamm/Wörterbuch).

| | Einkauf | To-Do |
|---|---|---|
| Store | `ShoppingStore` | `TodoStore` |
| Datei | `einkauf-local.json` | `todo-local.json` |
| Archiv | `einkauf-archiv.json` | `todo-archiv.json` |
...
To-Do **darf nicht** in `ShoppingStore` / `AppState` / `Item` / `einkauf-local.json` / `einkauf-archiv.json` / `kind: einkauf-backup` / Einkauf-WC-Kinds liegen.

**Archiv (nur iPhone, App Group):** Beim **endgültigen Löschen** erledigter Einträge (Swipe im Edit **und** Overflow **Erledigte löschen** / `clearDone` / `clearCompleted`) wird ein Snapshot an `einkauf-archiv.json` bzw. `todo-archiv.json` **angehängt** — nie die History überschreiben. Envelope `{ v: 1, entries: [ { archivedAt: ISO8601, item: /* lokaler Item- bzw. Task-Snapshot */ } ] }`. Jedes gelöschte erledigte Item = eigener `entries[]`-Eintrag mit eigenem `archivedAt` (Bulk: gleicher Burst-Zeitpunkt ok). Offene Deletes ohne Archiv; bloßes Abhaken schreibt nichts. Fehlende/leere Datei → `{v:1, entries:[]}`. Kein Watch-Sync, kein Complication-/Widget-Reload wegen Archiv, keine Archiv-Ansicht in den Listen (Einkauf/To-Do), kein Archiv-Import, **kein** „Archiv leeren“ / Clear-All. In **Einstellungen** je Sektion **Einkauf** und **To-Do**: Navigation **Archiv…** (`ArchiveListView`, `entries` neueste zuerst, Titel = Item-Name / Task-Text, darunter lesbares `archivedAt`; **Zurückspielen** legt eine **offene Kopie** auf die Live-Liste — neue `id` / neue `uid`, Einkauf `done = false` mit Dept/`urgency` aus dem Snapshot, To-Do `completed = false` mit Text/Person/Prio/`listId` soweit vorhanden; **Archiv-Zeile bleibt**; Wischen oder Papierkorb = **Dauerhaft löschen** eines Eintrags am Datei-Index, atomar; leer: „Noch keine Archiv-Einträge.“) plus **Archiv teilen** (Share Sheet analog Backup). Gelöschte History-Zeilen kommen nicht zurück, außer dasselbe Item wird später erneut erledigt und gelöscht.
...
- Overflow **…** (`ellipsis.circle`, „Mehr“): Backup importieren / exportieren / teilen, **MD exportieren…** / **MD teilen** / **CSV exportieren…** / **CSV teilen**, **Liste teilen**, Erledigte löschen. `fileImporter` `.json,.md,.markdown,.csv`. MD/CSV dump die **volle Liste** aller Aufgaben (`TodoMarkdown` / `TodoCSV`), nicht Auge und nicht Listenfilter; Listenmitgliedschaft optional (MD-Comment `Liste Name | list:id`, CSV-Spalten `Liste` / `List-ID` nur wenn Listen existieren). Import ohne Listeninfo bleibt gültig. PDF **Liste teilen** folgt der **aktuellen Liste und** `todo.iphone.showCompleted`. Kein Geh-Modus. Dasselbe JSON-Backup (**Backup importieren…** / **exportieren…** / **teilen**) liegt zusätzlich unter **Einstellungen → To-Do Backup** (`fileImporter` nur `.json`); **Archiv…** / **Archiv teilen** (`todo-archiv.json`) nur in den Einstellungen, nicht im To-Do-**…**. MD/CSV bleiben nur im To-Do-**…**.

**Listen-Modus vs Edit (Swipe-Löschen nur im Edit-Modus):**

- Listen-Modus: Checkbox toggelt `completed` (**nur** der Kreis, nicht Name/ganze Zeile); **kein** Swipe-Löschen (`.deleteDisabled` + trailing `EmptyView()`, List-`.id` `todo-browse`). Text-Tipp und Leading-Swipe **Edit** öffnen `TodoEditSheet`.
- Edit-Modus: Swipe-Löschen nur via `.onDelete` (List-`.id` `todo-edit`); Name/Zeile (nicht der Kreis) öffnet dasselbe Sheet (Text, Person, Prio A/B, Datum) über `todos.update`; Chevron-Affordance. Abhaken bleibt **nur** am Kreis. Umschalten tauscht die List-Identität, damit SwiftUI keine löschbaren Zeilen wiederverwendet. Leading-Swipe **Edit** bleibt in beiden Modi.
...
**MD/CSV:** Export/Import `TodoMarkdown` / `TodoCSV`; Dateiname `todo-liste.md` / `todo-liste.csv` (gestempelt wie JSON). **Volle Liste**, unabhängig vom Auge und vom Listenfilter. Listenmitgliedschaft rückwärtskompatibel (optional). Dateien ohne Listeninfo importieren wie bisher. Roundtrip der Fixtures `todo-liste.md` / `todo-liste.csv` bleibt. Import über denselben `fileImporter` (`.json,.md,.markdown,.csv`) und dieselben Anhängen/Ersetzen-Alerts. Einkauf-Dateien (JSON `einkauf-*`, MD `# Einkauf`, CSV-Kopf `Abteilung`) abgelehnt.
...
`WatchTodoListView`: kompaktes `#uid` Badge + Text (+ kompakte Person/Prio/Datum; bei erledigt **Abgeschlossen-Datum** `geschlossen TT.MM.JJJJ`); Tippen **nur** auf die Checkbox toggelt `completed` (Name ist Anzeige, kein Toggle). Auge `todo.watch.hideCompleted` (nicht `einkauf.watch.hideCompleted`, nicht Backup, nicht iPhone). Geh-Modus filtert auf dieselbe aktuelle Liste wie das iPhone. Titel: Listenname (oder **Alle**) plus `oo/xx/yy` der gefilterten Sicht. **Kein** Edit, kein Prio-Picker, keine reopen-Pills, kein Import/Export, keine Suche, kein Sort, kein PDF/MD/CSV, keine Listen-Verwaltung.

### Complication

Eigene WidgetKit-Complication `TodoProgress`: Label = **Name der aktuellen Liste** (`TodoListFilter.title`); Fallback **Alle** bei leerer oder unbekannter ID (wie Listenmenü, Watch-Titel und iPhone-Widget `To Do (Alle)` — nicht das feste „To Do“). Offene Anzahl der **aktuellen Liste** bzw. bei 0 **erledigt**. Daten `todo-local.json` plus App-Group-Key `todo.currentListId`. Tap `einkauf://todo`. Details unter **Watch-To-Do-Complication**.
...
| iPhone-Widget | Target `EinkaufWidgets`, Bundle `net.tschelle.einkauf.widgets`, WidgetKit (iOS 17), Homescreen `systemSmall`/`systemMedium`/`systemLarge`, eingebettet in die iPhone-App. Nicht auf der Watch, nicht Sperrbildschirm. |
...
| iOS-UI | `Sources/iOS` — `EinkaufApp`, `ContentView`, `TodoListView`, `SettingsSheet`, `SavedListEditView`, `KeywordDictionaryView`, `ListPDF`, `TodoListPDF`, `ShareSheet`, `AppearanceSettings`, `HomeWidgetReload` |
...
| Persistenz | `einkauf-local.json` (`kind: "einkauf-local"`) und `todo-local.json` (`kind: "todo-local"`) im App Group `group.net.tschelle.einkauf`, Ordner `Einkauf/` (iPhone-App + Widgets bzw. Watch-App + Complications; Geräte-Container getrennt). Daneben native-only Archive `einkauf-archiv.json` / `todo-archiv.json` (Append gelöschter erledigter Einträge; kein WC). Fallback Application Support beim ersten Umzug. Decoder lehnen fremdes `kind` ab. Kein gemeinsames JSON, kein iCloud für die Stores. Inbox ist eine fremde Drive-Datei — siehe **iCloud-Inbox (Zweitgerät)**. |
...
Große Checkbox + Name, Durchstreichen wenn `done`. **iPhone:** Tippen **nur** auf die Checkbox toggelt erledigt (wie `editRow`); der Name ist nur Anzeige, kein Toggle. **Watch:** Checkbox/Name tippen bleibt Abhaken. Links nur Checkbox + Text — **kein** führender Import-Streifen. Rechts L→R: Dringlichkeits-Chip (`ItemUrgencyChip`, ⚡ / ↔ / ↓ für eilig / normal / später; `normal` ist **↔** (U+2194), nicht leerer Rahmen, nicht `<->`, nicht ○ und nicht –; **dieselbe kompakte Kapsel** wie ⚡/↓, `fixedSize`, kein Strecken; auf dem **iPhone** tippbar) dann ganz außen der optionale **teal** Import-Streifen (`theme.slate` / `ItemImportedMark`, **nicht** `theme.good` — Grün ist erledigt), nur wenn `imported`, read-only, kein Toggle, **rechtester** Trailing-Slot. Tippen auf den Chip **nur iPhone** (Geh+Edit): `urgent` → `normal` → `later` → `urgent` (`cycleItemUrgency`, `listRevision`+1). **Watch:** dieselben drei Chip-Zustände inkl. ↔ für `normal` (**nicht** ausblenden) plus Import-Streifen — nur Anzeige, kein Tap, kein `cycleItemUrgency`. Beide Marker kombinierbar. Kein Grip, kein Dept-Select, kein Löschen, Name nicht editierbar. **Kein Swipe-Löschen** (jede Zeile `.deleteDisabled(true)`, kein `.onDelete`, trailing `swipeActions` nur `EmptyView()` gegen das System-Delete). Flache `ForEach`-Zeilen (`WalkListRow` / `WalkLine`): Überschrift dann Artikel. **Keine** SwiftUI-`Section` — die verschluckt die Abteilungsreihenfolge beim Ladenwechsel. Zeilen-IDs enthalten Laden und Position (`storeId|index|…`). List-`.id` `walk|` bzw. `edit|` plus `currentStoreId` + Layout-Join, plus Gruppen-`.id` `einkauf-walk` / `einkauf-edit`, damit Geh-Modus die Edit-Liste (inkl. Swipe-Löschen) nicht wiederverwendet.
...
Je Artikel: Checkbox, Name (Tipp → Rename; leer/Abbrechen = keine Änderung), Dept-`Picker` (`DepartmentCatalog.knownIds`: Builtins plus `customDepartments`), Dringlichkeits-Chip (tippbar, Zyklus wie Geh-Modus), ganz rechts Import-Streifen (nur Anzeige, äußerster Trailing-Slot), Ziehen, Swipe-Löschen.
...
## Einstellungen (nur iPhone, Tab)

Titel **Einstellungen**. Dritter iPhone-Tab (`gearshape`), kein Sheet, kein Fertig. Watch ohne diesen Tab. Native-Sektionen **Allgemein**, **Einkauf**, **To-Do**. Gemeinsames Gerüst mit HTML bleibt in **Einkauf** nach Backup/Inbox: Aktueller Laden → Neuer Laden → Ladenweg → **Eigene Abteilungen** (nur native) → Abteilungen hinzufügen → Stamm → Gespeicherte Listen → Wörterbuch. HTML schiebt **Alle Läden** JSON zwischen Neuer Laden und Ladenweg; Native hat dort nichts. HTML hat in v1 keine UI für `customDepartments`.

1. **Allgemein** — segmented Hell / Dunkel / System; segmented Creme / Blau. Footer: „Creme ist das Vintage-Papier, Blau die Navy-Palette. System folgt der iPhone-Einstellung für Hell und Dunkel.“ UserDefaults `einkauf.theme` / `einkauf.palette` (nicht im Backup).
2. **Einkauf** — **Backup importieren…** / **Backup exportieren…** / **Backup teilen** (`einkauf-backup`); **Archiv…** (Liste, neueste zuerst, **Zurückspielen** + Dauerhaft löschen) / **Archiv teilen** (`einkauf-archiv.json`); **Inbox verbinden…** / **Inbox abrufen** (dieselben Pfade wie Einkauf-**…**). Listen-**…** behält Backup/Inbox. **Kein** „Archiv leeren“.
3. **Aktueller Laden** — `ForEach(store.stores)`, Tippen wählt (`setStore`), Checkmark am aktuellen. **Swipe-Delete nur für `builtin == false`**. Builtin: `deleteDisabled`. Bestätigung: „Laden „{Name}“ wirklich löschen?“ Builtin-Seeds werden nie gelöscht (`StoreCatalog.delete` gibt `nil`). Löschen des aktuellen eigenen Ladens fällt auf `edeka` zurück; Seeds mergen.
4. **Neuer Laden** — Feld „Name des Ladens“ max 60, Anlegen. Footer: „Übernimmt das Layout des ausgewählten Ladens.“ (`StoreCatalog.create` kopiert `currentStore.layout`, `builtin: false`, id `s`+time36+random). Kein separater Knopf „Laden löschen“ zwischen Neuer Laden und Ladenweg.
5. **Ladenweg · {Name}** — `StoreLayout.sanitized`; `vor`/`nach` locked (`moveDisabled`, keine Pfeile/Entfernen). `sonstiges` ist **verschiebbar** (nicht locked) und folgt dem Layout-Slot — **geteilt mit HTML** (`groupItems` / `walkLayout`). `onMove` plus Nach oben / Nach unten / Entfernen für nicht-locked. Footer-Text aktuell: „Vor dem Einkauf immer vorn, Nach dem Einkauf immer hinten, Sonstiges direkt davor.“ — das lockt `sonstiges` **nicht**. HTML-Hinweis: „Sonstiges folgt der Position im Ladenweg.“ Regeneratoren dürfen `sonstiges` nicht an `nach` festkleben.
6. **Eigene Abteilungen** — Liste `state.customDepartments` (`{ id, title }`). Leer: „Noch keine eigenen Abteilungen.“ Zeile = Navigation **Abteilung bearbeiten** (Name max 60, Speichern = Rename, ID bleibt). Unten Feld „Name der Abteilung“ + **Anlegen**. Swipe-Delete und Löschen im Editor mit Confirm „Abteilung „{Titel}“ wirklich löschen?“. Löschen: alle Items, Stamm-Artikel, Saved-List-Einträge und `mappings` mit dieser `dept` → `sonstiges`; Abteilung aus **allen** Store-Layouts entfernen; Catalog-Eintrag weg. Builtins sind nicht löschbar. Neu angelegte IDs erscheinen unter **Abteilungen hinzufügen**, nicht automatisch im Ladenweg. Kein Keyword-Wörterbuch für eigene IDs (später).
7. **Abteilungen hinzufügen** — unused laut `StoreLayout.unused` inkl. eigener IDs; leer: „Alle Abteilungen sind im Layout.“ Sonst Button je Titel (`DepartmentCatalog.title`), Insert vor `nach`.
8. **Layout zurücksetzen** — builtin: Seed-Layout; eigener Laden: `["vor", "sonstiges", "nach"]`. Auch für Custom, nicht nur builtin. Eigene Abteilungen bleiben im Catalog, fallen nur aus dem Layout.
9. **Stamm-Artikel** — Name, Dept-Picker (`DepartmentCatalog.knownIds`), Löschen; Anlegen „Milch, Butter…“. Footer zu Gesamtliste. `onMove` plus Nach oben / Nach unten — Nutzer-Reihenfolge im `staples`-Array, kein Auto-ABC. Dieselbe Reihenfolge im Menü **Stamm** und bei **Gesamtliste**.
10. **Gespeicherte Listen** — leer: „Noch keine gespeicherten Listen.“ Sonst NavigationLink **Liste bearbeiten** (`SavedListEditView`: Name-Feld max 60, Artikel Name+Dept inkl. `DepartmentCatalog.knownIds`, Einträge löschen, neue Einträge anlegen). Separater Button **Übernehmen** = `applySavedList` (auffüllen, nicht ersetzen) — Tippen auf den Namen öffnet die Vorlage, nicht Apply. Swipe-Delete mit Confirm „Gespeicherte Liste „{Name}“ wirklich löschen?“ Footer: Anlass-Listen, Bearbeiten öffnet die Vorlage, Übernehmen füllt auf ohne zu ersetzen, Wischen löscht.
11. **Wörterbuch** — NavigationLink, lokal `KeywordDictionary.source`, Suche „Wort suchen“ filtert **Meine Zuordnungen** und die mitgelieferten Wörter. Oben Sektion **Meine Zuordnungen**: Zeilen aus `state.mappings` (Key = `mappingKey`/Canon ohne Menge + Dept-Picker `DepartmentCatalog.knownIds`, damit eigene Depts nicht nach `sonstiges` umgeschrieben werden). Swipe-Delete ruft `removeMapping`. Leer: „Noch keine eigenen Zuordnungen. Abteilung im Bearbeiten-Modus ändern — dann erscheint der Name hier.“ Optional **Hinzufügen** (Name + Dept → `setMapping` / `mappingKey(name)`). Darunter die mitgelieferten Gruppen nur lesen (Builtins, kein Keyword-Dictionary für eigene Abteilungen). Footer: mitgelieferte Wortliste ist fest; eigene Zuordnungen stehen im Backup als `mappings` und gewinnen beim nächsten Eintragen; Sonderregeln (TK, Eistee, Schorle, Chips, Eis) stehen nicht in dieser Liste; Korrekturen unter dem Artikelnamen (ohne Menge); das Wörterbuch selbst ändert sich nicht. Kein Netz. `setMapping` / `removeMapping` persistieren und WatchConnectivity wie andere State-Änderungen (inkl. Home-Widget-Reload).
12. **To-Do** — native-only (**To-Do Backup**). **Backup importieren…** / **Backup exportieren…** / **Backup teilen** für JSON `format: "todo-v3-json"` (`todo-liste.json`). **Archiv…** (Liste, neueste zuerst, **Zurückspielen** + Dauerhaft löschen) / **Archiv teilen** analog für `todo-archiv.json` (`yyyyMMdd_HHmm-todo-archiv.json`). `fileImporter` nur `.json`. Derselbe `TodoImport.offer` / `TodoStore.importAny`-Pfad wie To-Do-**…** und `onOpenURL`: leer → direkt setzen, sonst **Anhängen** / **Ersetzen**. Import hebt `revision` über den Peer (`max(lokal, import) + 1`) und synct. Einkauf-JSON (`kind: "einkauf-backup"`) wird mit deutschem Fehler abgelehnt, nie in `ShoppingStore` geschrieben. MD/CSV bleiben im To-Do-Overflow. To-Do-**…** Backup importieren bleibt. Archiv-JSON wird in dieser Version nicht importiert. **Kein** „Archiv leeren“.

Kein Theme in der Watch-App. Watch-Root: Palette Vintage + System-`colorScheme` (iPhone-Darstellung synct nicht).

## Watch

`TabView` **Einkauf | To-Do** (`basket` / `checklist`). Kein dritter Tab **Einstellungen**. Einkaufs-Tab: nur Geh-Modus, wie bisher. To-Do-Tab: nur Geh-Modus — kompaktes `#uid` Badge + Text (+ kompakte Person/Prio/Datum; bei erledigt **Abgeschlossen-Datum**); Tippen **nur** auf die Checkbox toggelt `completed`; Auge `todo.watch.hideCompleted` (nicht `einkauf.watch.hideCompleted`, nicht Backup, nicht iPhone); Filter auf die vom iPhone gesyncte aktuelle Liste (`todo.currentListId`, leer = Alle). Kein Edit, kein Prio-Picker, keine reopen-Pills, kein Import/Export, keine Suche, kein Sort, keine Listen-Verwaltung auf der Watch. iPhone-To-Do: siehe **To-Do**.
...
Geh-Zeile: links nur Checkbox + Name. Rechts L→R: Dringlichkeits-Chip **immer alle drei Zustände wie iPhone** — ⚡ / ↔ / ↓ (auch `normal` sichtbar als ↔, **nicht** ausblenden; nur Anzeige, nicht tippbar, kein `cycleItemUrgency`; nicht leerer Rahmen, nicht `<->`, keine ○/–; **dieselbe kompakte Kapsel**) dann ganz außen der optionale teal Import-Streifen (`theme.slate`, nur Anzeige, nie editierbar, rechtester Slot). Checkbox/Name tippen bleibt Abhaken. Abteilungsüberschriften über `DepartmentCatalog.title` / `ShoppingStore.departmentTitle` (eigene Titel synct mit `customDepartments` im `AppState`). Gruppierung darf eigene IDs nicht nach `sonstiges` falten. Kein Store-Picker, kein Edit, kein Share, kein Backup, kein Speichern, kein Wörterbuch, kein Löschen von Läden, **kein** Anlegen eigener Abteilungen auf der Watch. Digital Crown scrollt die `List`. Leer: „Noch nichts auf der Liste.“ **Kein** In-App-Mikrofon, **kein** Diktat-Panel, **kein** `Speech.framework` / AVFoundation-Speech, **kein** `TextFieldLink` / `presentTextInputController`. Sprache nur über **Siri App Intents** — siehe **Sprach-Eingabe (Siri)**.
...
Zähler **nur** die offenen Artikel (`ComplicationSnapshot.compactCountText`): `"\(openCount)"`. Bei 0 offenen (leere Liste oder alles abgehakt): das Wort **erledigt**, nicht `0` und nicht `oo/xx/yy`. `progressLabel` (`oo/xx/yy` / `0/0/0`) bleibt für Watch-Titel, iPhone-Widget, PDF, HTML. Titel fest **Einkauf** (`ComplicationSnapshot.titleLabel`, nicht Ladenname, nicht „Einkaufsliste“) in Rechteck, Ecken-`widgetLabel` und Inline. Circular nur Zähler. Weiter antippbar. VoiceOver: „Einkauf, 5 offen“ bzw. bei 0 „Einkauf, Liste erledigt“. Der gekürzte Ladenname (`clippedWatchStoreName`) bleibt nur im Watch-App-Titel `watchTitle`.

`accessoryCircular` darf **kein** großes System-`Text` (`.title2` / `.title3` / `.title`) für den Zähler nutzen: auf der physischen Watch passt das in Infograph/Modular compact nicht, `minimumScaleFactor` rettet accessory-Families oft nicht, und watchOS zeichnet dann ein alleinstehendes **!** statt zu kürzen. Stattdessen Gauge 0…1 (`done/total`, leer = 0) mit kleinem Zentrum (`caption`/`caption2`/ca. 11–13pt, **eine** Zeile offene Anzahl bzw. „erledigt“ — kein dreizeiliges `oo`/`xx`/`yy`). `accessoryCorner`: `compactCountText` **größer** als der Titel **Einkauf** (~19pt semibold rounded vs. Caption/~11pt im `widgetLabel`), damit die offene Anzahl unter dem gebogenen Titel lesbar bleibt; `minimumScaleFactor` für „erledigt“. Rectangular schon großer Zähler (`.title2`). Alle Families: `.containerBackground` (watchOS 10).
...
| `accessoryRectangular` | Titel **Einkauf** + offene Anzahl bzw. „erledigt“ |
| `accessoryInline` | **Einkauf** + Zähler, sonst nur Zähler (`ViewThatFits`; 0 → „erledigt“) |
| `accessoryCorner` | großer Zähler (~19pt, 0 → „erledigt“), kleiner Titel **Einkauf** im `widgetLabel` (~11pt) |
...
Label = **aktueller Listenname** (`TodoListFilter.title` aus `TodoState.lists` + `currentListId`). Fallback **Alle** bei leerer oder unbekannter ID — dieselbe Semantik wie das iPhone-Listenmenü, der Watch-Titel und das Homescreen-Widget `To Do (Alle)`. Langer Name: `lineLimit(1)` + `minimumScaleFactor`, plus Kürzung analog `clippedWatchStoreName`. Zähler nur offene Aufgaben der **aktuellen Liste** (`TodoComplicationSnapshot.make(from:currentListId:)`, `TodoCurrentList.syncedId`); bei 0 **erledigt** (auch wenn andere Listen noch offen sind). Corner: offene Anzahl ~19pt, Listenname ~11pt im `widgetLabel` (wie Einkauf-Ecke). Circular: Gauge, kleines Zentrum, kein `.title2` — nur die Zahl, kein Listenname. `configurationDisplayName` bleibt **To Do** (Widget-Galerie). Datenquelle `todo-local.json` (`TodoPersistence` / `TodoCodec`, `kind: "todo-local"`) plus App-Group `todo.currentListId`. Reload: `WatchComplicationReload.todoTimelines()` nach `TodoStore`-Persist und nach WC-`currentListId`. iPhone-Homescreen-Widget kombiniert Einkauf + To-Do (aktuelle Liste); Labels dort unverändert (`Einkaufsliste` / `To Do (<Liste>)`); keine eigene To-Do-only Complication auf dem iPhone.

### iPhone-Widget (WidgetKit, iOS 17)

Nur auf dem **iPhone-Homescreen**, nicht auf der Watch und nicht auf dem Sperrbildschirm. Zwei Domains in **einem** Widget: Einkauf und To-Do der **aktuellen Liste** (`todo.currentListId` / AppStorage `todo.iphone.currentListId`, in die App Group gespiegelt). **Keine** Aufschlüsselung nach mehreren To-Do-Listen (auch nicht in Large). Zähler überall `oo/xx/yy` = offen / erledigt / gesamt (wie App / `AppState.progressLabel` / `TodoListFilter.progressLabel`, Einkauf inkl. `vor`/`nach`). Leere Domain: `0/0/0`, weiter antippbar.

| Family | Inhalt |
|---|---|
| `systemSmall` | Zwei **gestapelte Blöcke** (Abstand dazwischen, Leerzeilen-Gefühl). Pro Domain: Label in **eigener Zeile**, darunter `oo/xx/yy` volle Breite — nicht Label und Zähler nebeneinander. Einkauf: `Einkaufsliste` (passt nicht → **Einkauf**, `ViewThatFits`); To-Do: `To Do (<Listenname>)`. Labels Caption, `lineLimit(1)`. Zähler Headline rounded semibold, monospaced, unter dem Label. Leere oder unbekannte Listen-ID → `To Do (Alle)`. Ein Tap (`widgetURL` `einkauf://list`). |
| `systemMedium` / `systemLarge` | Mini-Tabelle: Zeilen **Einkauf** und **To Do (<Listenname>)**, Spaltenköpfe **Offen** / **Erledigt** / **Gesamt**. Tippen auf die Einkaufs-Zeile → `einkauf://list`; To-Do-Zeile → `einkauf://todo`. |

Datenquellen über App Group `group.net.tschelle.einkauf` (kein iCloud, kein Netz): `einkauf-local.json` (`Persistence`) und `todo-local.json` (`TodoPersistence`) plus die aktuelle Listen-ID. Timeline-Provider liest beide Dateien (kaputtes/leeres JSON → Seed bzw. empty, kein Crash). Die iPhone-App spiegelt die Listenwahl (`TodoCurrentList.syncIphoneToAppGroup`) und fordert nach Einkauf-**und** To-Do-Persist sowie bei Listenwechsel einen Widget-Reload an. `HomeWidgetReload` zieht Requests zusammen (trailing Debounce ~250 ms, **ein** `reloadTimelines` pro Burst, nicht auf dem Main-Thread) — `persistQuietly` schreibt weiter sofort und noch einmal debounced (~80 ms); nur der Widget-Reload ist coalesced (Build 70, Hang-Fix). Fallback-Timeline alle 30 Minuten.

Kein Store-Picker, keine offenen Artikelzeilen, kein per-Liste-Raster.
...
**Geliefert (Phasen 1–3).** Haupt-iPhone: **Inbox verbinden…** (Dateien-Picker → Security-scoped Bookmark auf `inbox.txt`). **Inbox abrufen:** iCloud-Download anstoßen (`InboxCloudDownload` / `startDownloadingUbiquitousItem`, nur ubiquitäre URL; Fehler „läuft schon“ ignorieren), kurz pollen (`Task.sleep`, Timeout wenige Sekunden), dann Lesen (`Data(contentsOf:)`) → Auswahl-Sheet (`InboxRetrieveSheet`, alle markiert) → ausgewählte Zeilen `ShoppingStore.addItems(fromSpeech:imported: true)` → Datei nur noch Abgewählte (leer, wenn alle übernommen). Timeout liest den lokalen Stand; Fehler nur wenn die Datei wirklich unlesbar ist. Kein persistenter `NSMetadataQuery`. **Löschen** pro Zeile: Swipe `.onDelete` und Papierkorb, schreibt `inbox.txt` sofort ohne Import; letztes Item schließt das Sheet. Zwei Kurzbefehle auf dem **Zweit-iPhone** — **Einkauf-Inbox eintragen** und **Einkauf-Inbox vorlesen**, Schritt-für-Schritt unter **Kurzbefehl (Phase 3)**. Keine `.shortcut`-Binärdatei im Repo. **Nur Einkauf** — nie To-Do. Transport: eine geteilte **iCloud-Drive**-Datei. Kein Server, kein CloudKit Shared DB, kein Dropbox/kDrive, kein iCloud-Entitlement (Files-Picker + Bookmark). **Optional offen:** Phase 4 (concurrent Append) — kein v1-Ziel.
...
Wann / wo / wie — nur die Schritte, die **du** machst. **Inbox verbinden…** und **Inbox abrufen** (Auswahl + **Löschen** ohne Import) sind in der App. Die zwei Kurzbefehle baust **du** einmal auf dem Zweit-iPhone nach dem Rezept **Kurzbefehl (Phase 3)**.
...
4. **Inbox verbinden…** **Wann:** jetzt. **Wo:** Einkauf-App auf dem **Haupt-iPhone**, Overflow **…** (nicht To-Do). **Wie:** einmal **Inbox verbinden…** → Dateien-Picker auf genau diese `inbox.txt` (`.plainText` / `public.text` / `.txt`, Security-scoped Bookmark). Nicht To-Do-Dateien, nicht ein Backup-JSON. Danach **Inbox abrufen** (Auswahl; **Löschen** ohne Import).
5. **Kurzbefehle bauen.** **Wann:** jetzt (Rezept unten). **Wo:** Zweit-iPhone, App **Kurzbefehle**. **Wie:** einmal die zwei Kurzbefehle **Einkauf-Inbox eintragen** und **Einkauf-Inbox vorlesen** nach **Kurzbefehl (Phase 3)** anlegen; beide auf **dieselbe geteilte** `inbox.txt` (Anhängen per **An Textdatei anhängen**, nicht „Dateien auswählen + Datei sichern“). Keine Binärdatei zum Import — Aktion für Aktion nachbauen.

Ohne Schritt 1–3 funktioniert nichts. Schritt 4 und 5 sind in Build 62 enthalten; Schritt 5 ist das Rezept in dieser Datei.

#### Alltag Zweit-iPhone

- **Einkauf-Inbox eintragen** → Artikel diktieren → die Phrase wird als eine Zeile an `inbox.txt` **angehängt**. Optional „Artikel vorgemerkt.“
- **Einkauf-Inbox vorlesen:** alles, was in der Datei steht, ist **noch nicht abgeholt**. Datei bleibt unverändert. Kein „erledigt“ in der Datei. Leer → „Nichts abzuholen.“
...
- Phasen 1–3: App-UI (**Inbox verbinden…**, Bookmark, **Inbox abrufen** mit Auswahl, **Löschen** pro Zeile ohne Import) und zwei Kurzbefehle — Rezept unter **Kurzbefehl (Phase 3)** (kein `Docs/Einkauf-Inbox-Kurzbefehl.md`, keine `.shortcut`-Binärdatei). Parser-Grundtests (leer, Kommentare, Items, BOM) plus Partition ausgewählt/Rest plus Entfernen/Selection-Shift.
- **Optional offen — Phase 4:** concurrent Append während des Abrufs, Bookmark ungültig. Kein v1-Ziel.

### Kurzbefehl (Phase 3)

Einmal auf dem **Zweit-iPhone** in der App **Kurzbefehle** nachbauen. **Zwei** getrennte Kurzbefehle — bestätigt auf **iOS 26** (Gerät):

1. **Einkauf-Inbox eintragen** — diktieren und an dieselbe `inbox.txt` **anhängen**
2. **Einkauf-Inbox vorlesen** — dieselbe Datei vorlesen, ohne sie zu ändern

Kein `.shortcut` im Repo. Aktionsnamen wie in iOS 26 auf Deutsch (englische Bezeichnung in Klammern, wo die Suche hilft). Suche jede Aktion über das **+** / Suchfeld.

**Nicht im Kurzbefehl teilen.** Eine diktierte Phrase wird **eine Zeile** (so wie **An Textdatei anhängen** sie schreibt). Die Haupt-App **Inbox abrufen** führt `InboxParser` + `addItems(fromSpeech:)` / `SpeechItemSplitter` aus (Komma, Semikolon, ` und `, ` sowie `, Zeilenumbruch). Der Kurzbefehl schreibt **keinen** `#`-Header. Die Datei bleibt **nur noch nicht Abgeholte**.

Altes Rezept (ein Menü-Kurzbefehl **Einsprechen** / **Vorlesen**, **Datei holen**, **Text ersetzen** / **Text teilen**, Read-Modify-Write per **Datei sichern**) nicht nachbauen — auf iOS 26 gilt nur der Ablauf unten.

#### A. Einkauf-Inbox eintragen

**Wann:** einmal anlegen, danach im Alltag zum Vormerken. **Wo:** Zweit-iPhone, App **Kurzbefehle**. **Wie:**

1. **+** → Name oben **Einkauf-Inbox eintragen** (nicht „besorgen“, nicht „Todo“).
2. Aktion **Text diktieren** suchen und einfügen. Anhalten, wenn die Phrase fertig ist. Beispiel: „Milch, Butter und zwei Eier“ — bleibt **eine** Zeile.
3. Aktion **An Textdatei anhängen** (iOS EN: **Append to Text File**). Suche `anhängen` / `anfügen` / `Append`. **Nicht** „Dateien auswählen + Datei sichern“ zum Anhängen.
4. Modus **Anfügen** (Append) an. **Neue Zeile** an.
5. Speicherort: oft Default **Kurzbefehle**. Darauf tippen → **Ersetzen** (Replace) → geteilten Ordner **Einkauf-Inbox** in iCloud Drive (**Geteilt**) wählen → **Öffnen**. Das ist derselbe Ordner wie auf dem Haupt-iPhone, nicht eine Kopie unter **Kurzbefehle**.
6. Dateipfad / Dateiname genau **`inbox.txt`** (mit `.txt`).
7. Optional **Text sprechen** „Artikel vorgemerkt.“
8. Erster Lauf kann Mikrofon- und Dateizugriff verlangen — erlauben.

Ohne **Ersetzen** auf den geteilten Ordner schreibt der Kurzbefehl ins eigene iCloud Drive / **Kurzbefehle**. Das Haupt-iPhone sieht dann nichts.

**Append:** **An Textdatei anhängen** mit **Anfügen** + **Neue Zeile** ist der bestätigte Weg. Existiert `inbox.txt` im gewählten Ordner, wird die Phrase angehängt. Existiert sie nicht, legt die Aktion eine **neue** Datei an — dann muss der Ort der geteilte `Einkauf-Inbox`-Ordner sein, sonst entsteht eine Parallel-`inbox.txt` unter **Kurzbefehle**.

Gleichzeitiges **Inbox abrufen** auf dem Haupt-iPhone während des Anhängens kann Zeilen verlieren (Phase 4). Im Alltag: erst eintragen und warten, dann auf dem Hauptgerät abrufen.

#### B. Einkauf-Inbox vorlesen

**Wann:** nach dem Eintragen, bevor das Haupt-iPhone abruft. **Wo:** Zweit-iPhone, App **Kurzbefehle**. **Wie:** dieselbe Datei wie eintragen — derselbe Ordner `Einkauf-Inbox`, dieselbe `inbox.txt`. Vorlesen ändert die Datei nicht.

1. **+** → Name **Einkauf-Inbox vorlesen**.
2. Aktion **Dateien auswählen**. Einmal auf die geteilte `inbox.txt` zeigen (iCloud Drive → **Geteilt** → `Einkauf-Inbox` → `inbox.txt`). **Jedes Mal fragen** aus. Datei-Chip sichtbar.
3. Text holen (Datei-Inhalt; Suche „Text holen“ / „Text aus Eingabe holen“).
4. **Wenn** leer / kein Wert: **Text sprechen** „Nichts abzuholen.“ **Sonst:** **Text sprechen** den Dateiinhalt.

Gleichwertig ist jede Kette, die **dieselbe** Datei liest (gleicher Ordner, gleiche `inbox.txt`) und spricht — ohne die Datei zu schreiben.

Alles Vorgelesene ist weiterhin **noch nicht abgeholt**, bis das Haupt-iPhone **Inbox abrufen** tippt.

#### C. Siri und Homescreen (optional)

- Je Kurzbefehl-Details (**ⓘ**): **Mit Siri verwenden** — z. B. „Einkauf Inbox eintragen“ / „Einkauf Inbox vorlesen“. Nicht **besorgen**, nicht **Todo**.
- Optional **Zum Home-Bildschirm**.

#### D. Schnelltest (ohne Raten)

1. Zweit-iPhone: **Einkauf-Inbox eintragen** → „Milch, Butter und zwei Eier“. Optional Sprache: „Artikel vorgemerkt.“
2. App **Dateien** auf dem Zweit-iPhone: geteilte `Einkauf-Inbox/inbox.txt` hat **eine** Zeile `Milch, Butter und zwei Eier` — angehängt, nichts überschrieben, kein `#`-Header. Nicht die Kopie unter **Kurzbefehle**.
3. **Einkauf-Inbox vorlesen** spricht diese Zeile (nicht „Nichts abzuholen.“). Datei unverändert.
4. Haupt-iPhone, Einkauf-**…** → **Inbox abrufen** → Auswahl-Sheet, die Zeile markiert → **Übernehmen** → „3 Artikel übernommen.“ Liste hat die drei Artikel (Abteilung wie Siri **besorgen**; Split in der App). Datei danach leer. Eine Zeile abwählen → die bleibt in `inbox.txt`. **Löschen** einer Zeile entfernt sie aus der Datei, ohne sie zu importieren.
5. Zweit-iPhone **Einkauf-Inbox vorlesen** → „Nichts abzuholen.“
6. Noch einmal **eintragen** „Brot sowie Milch“, danach erneut **eintragen** „Eier“. Datei hat zwei Zeilen — das zweite Anhängen hat die erste **nicht** gelöscht.

#### E. Wenn etwas schiefgeht

| Symptom | Ursache | Fix |
|---|---|---|
| Haupt-iPhone „Nichts abzuholen.“, Dateien / Zweit-iPhone zeigen Zeilen | iCloud-Datei noch nicht lokal; Abruf las den Placeholder | Build 73: `startDownloadingUbiquitousItem` + kurzes Poll vor `Data(contentsOf:)`; Timeout liest den lokalen Stand |
| Haupt-iPhone „Nichts abzuholen“, Dateien-App auf dem Zweitgerät zeigt die Zeilen | Kurzbefehl hat in den Default-Ordner **Kurzbefehle** geschrieben, nicht in die geteilte Datei | Speicherort **Ersetzen** → **Geteilt** / `Einkauf-Inbox`; Dateiname genau `inbox.txt` |
| `inbox.txt` fehlt unter Geteilt, taucht aber unter **Kurzbefehle** auf | **An Textdatei anhängen** hat eine neue Datei im eigenen Ordner angelegt | Ordner auf geteiltes `Einkauf-Inbox` **ersetzen**; Dateiname mit `.txt` |
| Dateiname ohne Endung / `inbox` statt `inbox.txt` | Aktion legt `inbox` ohne `.txt` an — App verbindet `inbox.txt` | Dateipfad genau **`inbox.txt`** |
| Alte vorgemerkte Zeilen weg nach dem zweiten Eintragen | nicht **Anfügen**, oder doch **Datei sichern** mit Überschreiben | Aktion **An Textdatei anhängen**, Modus **Anfügen**, **Neue Zeile** an |
| Nach Abruf nur ein Artikel „Milch, Butter und zwei Eier“ | Split fehlt in der App (nicht im Kurzbefehl) | Haupt-App **Inbox abrufen** muss `addItems(fromSpeech:)` / `SpeechItemSplitter` nutzen |
| Siri nimmt Bring / die App **besorgen** | Phrase kollidiert | Siri-Name **Einkauf Inbox eintragen** / **vorlesen**, nicht **besorgen** |

### Phasen

- [x] **Phase 1** — Spec nur hier in `Description.md` (kein `Docs/InboxIntegration.md`, kein Build-Bump).
- [x] **Phase 2** — App-UI auf dem Haupt-iPhone (Build 59): **Inbox verbinden…** (Dateien-Picker → `inbox.txt`). **Inbox abrufen** (Build 60): Auswahl-Sheet, ausgewählte → `addItems(fromSpeech:)`, Abgewählte bleiben in der Datei. **Löschen** pro Zeile ohne Import (Build 61). iCloud-Download vor dem Lesen (Build 73). Nur Einkauf-Tab. Parser-Grundtests.
...
## Abteilungen `Department` + `DepartmentCatalog`

Builtin-IDs nicht ändern: `vor` Vor dem Einkauf; `obst` Obst & Gemüse; `brot` Brot & Backwaren; `bedienung` Fleisch, Wurst, Käse; `kuehlung` Kühlregal; `tiefkuehl` Tiefkühl; `trocken` Trockenwaren; `suess` Süßwaren & Snacks; `getraenke` Getränke; `drogerie` Drogerie & Haushalt; `sonstiges` Sonstiges; `nach` Nach dem Einkauf.

`Department.resolved` ohne Customs: Unbekannt → `sonstiges`. **`DepartmentCatalog.resolved(_:customs:)`** lässt eigene IDs aus `customDepartments` stehen; nur IDs, die weder Builtin noch Custom sind, werden `sonstiges`. `Item.init(from:)` schreibt die Roh-ID (leer → `sonstiges`) und remappt nicht — `BackupCodec.normalized` / `decode` löst mit Customs auf. **`item.dept` beim Gruppieren nicht nach `sonstiges` umschreiben**, wenn die Abteilung bekannt (Builtin oder Custom), aber nicht im Layout ist.

`customDepartments`: `{ id, title }[]`. id `d`+time36+random, nicht Builtin, unique. Titel max 60, sichtbar; Rename ändert nur den Titel. Alte Backups ohne Feld → `[]`.

`DepartmentCatalog.knownIds` = Builtin-`allCases` plus Custom-IDs in Array-Reihenfolge. `title(for:customs:)` Builtin-Titel, sonst Custom-Titel, sonst die Roh-ID. Store-`layout` bleibt `[String]` und darf Custom-IDs halten (`StoreLayout.sanitized` droppt sie nicht).

### `ListGrouping.groups`

1. Layout = `StoreLayout.sanitized` (`vor` zuerst, `nach` zuletzt, Duplikate/Unbekannt raus; Custom-IDs bleiben; **`sonstiges` bleibt an der Layout-Position**).
2. Nur Gruppen mit Artikeln.
3. Layout-Gänge außer `nach` in Layout-Reihenfolge (inkl. `sonstiges` und eigener IDs, wo sie stehen).
4. Extra-Abteilungen **mit Items**, die nicht im Layout stehen, danach — vor `nach` (`DepartmentCatalog.knownIds`-Reihenfolge: Builtins, dann Customs).
5. `nach` zuletzt.
6. Innerhalb: `ord`, dann `added`, dann `localeCompare` de.
7. `DeptGroup.title` kommt vom Catalog (Watch und iPhone dieselbe Quelle).
...
`AppState`: `currentStoreId`, `stores`, `items`, `mappings`, `walkMode`, `staples`, `savedLists`, `customDepartments`, intern `listRevision`.
...
1. **`mappings[mappingKey(name)]`**, falls bekannte Dept (Builtin oder Custom) — Nutzerkorrektur gewinnt für genau diesen Mapping-Key
...
`staples`: `{ name, dept }[]`. `sanitizeStaples` akzeptiert alte Strings und Objekte; Duplikate via `mappingKey`; ungültiges `dept` → `guess`. Array-Reihenfolge ist Nutzer-Order (Einstellungen Drag/`onMove` und Pfeile, `persistAndSync`); Menü **Stamm** und `applyAllStaples` folgen derselben Reihenfolge. Kein erzwungenes ABC. `applyStaple` / `applyAllStaples` wie HTML: fehlend anlegen, erledigt wieder öffnen, schon offene zählen/`already`. Anlegen: gleicher `mappingKey` = no-op, neuer Eintrag ans Ende.
...
Apply = `StapleApply.applyAll` auf die Snapshot-Items: **füllen, nicht ersetzen**. Offene bleiben, Matching per `mappingKey` öffnet erledigte wieder, fehlende kommen dazu. Overflow-Menü **Gespeicherte Listen** bleibt Tippen = Apply. In **Einstellungen** ist Apply der Button **Übernehmen**, damit Edit und Apply nicht kollidieren.

**Bearbeiten (nur iPhone-Einstellungen, `SavedListEditView`, Titel Liste bearbeiten):** `renameSavedList` (Name max 60, leer/ungültig = no-op), `renameSavedListItem` (leer = keine Änderung), `setSavedListItemDept` (schreibt `mappings` wie Stamm), `removeSavedListItem` (auch letzter Artikel; leere Vorlage bleibt bis Sanitize), `addSavedListItem` (Abteilung per Guesser). Snapshot-Shape bleibt `{ name, dept }` — kein `done`/`id`/`imported`/`urgency` in der Vorlage. To-Do-Listenmenü unverändert.
...
Lokal: `kind: "einkauf-local"` mit internem State (Revision, `doneChangedAt`, `savedLists`, `customDepartments`). Alte Dateien ohne `customDepartments` → `[]`.
...
```
{
  ...
  todo:    { kind: "todo-sync",    v: 1, blob, currentListId }
}
```
...
Native stellt **Allgemein** (Hell/Dunkel/System, Creme/Blau) davor. HTML hat **kein** Darstellung-Block im Sheet; Theme/Palette bleiben am Site-Mast. HTML schiebt **Alle Läden** JSON (`kind: "einkauf-laeden"`) zwischen Neuer Laden und Ladenweg — nur HTML. Native hängt **Einkauf** (Backup, Archiv-Liste + Teilen, Inbox) und **To-Do** (To-Do Backup) an den Tab **Einstellungen**. **Eigene Abteilungen** (`customDepartments`) sind native-only inkl. Watch-Titel/Gruppierung; das Backup-Feld ist für späteres HTML da, die PWA-UI kommt nicht in diesem PR.
...
**Nur native / nicht ins HTML:** Watch (Geh-Modus, Titel gekürzter Laden + Einkauf oo/xx/yy, Erledigte ausblendbar, WidgetKit-Complication nur offene Anzahl bzw. „erledigt“, kein Picker/Edit/Share, **kein** In-App-Mikro); iPhone-Geh-Modus mit eigenem Auge (Edit ungefiltert); **iPhone-Homescreen-Widget** (`systemSmall`/`systemMedium`/`systemLarge`, Einkauf + To-Do der aktuellen Liste, `oo/xx/yy`); **Siri / App Intents** für Einkauf (**besorgen** + „o“) und To-Do (**Todo**, iPhone „o“, Watch ohne `requestValueDialog`); **PDF Liste teilen** folgt dem iPhone-Auge (`einkauf.iphone.hideCompleted` bzw. To-Do `todo.iphone.showCompleted`), leere quadratische Kästchen; Erscheinungsbild **System** (folgt iPhone-Appearance) plus Hell/Dunkel und Creme/Blau **nur in Einstellungen**, nicht in der Toolbar; **To-Do**-Tab, Watch-Geh-To-Do, Complication **To Do**; **iCloud-Inbox** (Einkauf-**…**: **Inbox verbinden…** / **Inbox abrufen**, Bookmark auf `Einkauf-Inbox/inbox.txt`; Abruf-Sheet **Übernehmen** plus **Löschen** pro Zeile ohne Import; Zweit-iPhone-Kurzbefehle **Einkauf-Inbox eintragen** / **Einkauf-Inbox vorlesen** nach **Kurzbefehl (Phase 3)**); **Archiv** gelöschter erledigter Einträge (`einkauf-archiv.json` / `todo-archiv.json`, Liste + **Zurückspielen** + Einzel-Löschen und Teilen in Einstellungen, kein Clear-All). TestFlight nicht erforderlich. Native-`guess` wertet Nutzer-Mappings vor Sonderregeln/Keywords aus (HTML-Reihenfolge kann abweichen).

Native To-Do liefert MD/CSV wie HTML (**volle Liste**) und benannte Listen (`lists`/`listId`). To-Do-JSON-Backup zusätzlich unter **Einstellungen → To-Do Backup** plus **Archiv…** / **Archiv teilen**. Import-`revision`-Floor analog Einkauf. iPhone-Zeile `#uid` Badge + reopen-Pills wie HTML. iPhone-Listen ohne großen Nav-Titel, Toolbar kompakt. **Kein** HTML-Parity für Site-Mast; HTML-Listen folgen dem hier definierten `todo-v3-json`-Schema in einem separaten Pages-PR.
...
- Complication nur Watch/WidgetKit, nicht iPhone, kein ClockKit, kein iCloud. Einkauf-Complication-Titel fest **Einkauf** (nicht Laden, nicht „Einkaufsliste“). To-Do-Complication-Label = aktueller Listenname, Fallback **Alle**; Zähler offene Anzahl / **erledigt**, Datei `todo-local.json`
- iPhone-Widget nur Homescreen (`systemSmall`/`systemMedium`/`systemLarge`), nicht Watch, nicht Sperrbildschirm, kein iCloud; To-Do nur aktuelle Liste, kein per-Liste-Raster, kein separates To-Do-only-Widget
- Theme nur in Einstellungen; To-Do-Tab teilt `einkauf.theme` / `einkauf.palette`
- PDF-Kästchen leer (kein Fill, kein Häkchen)
- To-Do Isolation: eigener `TodoStore`, `todo-local.json` / `kind: "todo-local"`, Backup `todo-v3-json`; nie in `ShoppingStore` / `einkauf-local.json` / `einkauf-archiv.json` / `kind: einkauf-backup`. WC merget `{einkauf, todo}`
- Archiv nur native iPhone: Append in `einkauf-archiv.json` / `todo-archiv.json` beim Löschen erledigter Einträge; in **Einstellungen → Archiv…** Dauerhaft löschen (einzelner Eintrag) und **Zurückspielen** (offene Kopie, Archiv bleibt); kein Clear-All / „Archiv leeren“, kein HTML, kein WC, keine Listen-UI (Einkauf/To-Do), kein Import in dieser Version
...
- iPhone-Hauptlisten ohne Navigationstitel, Toolbar kompakt (`einkaufToolbarChrome`); Sheets behalten ihren Titel
- Listen nativ: optionales `lists`/`listId`, Filter **Alle**, PDF+Siri+Watch folgen `currentListId`; HTML-Site nicht in diesem Repo
...
- [ ] Bundle `net.tschelle.einkauf`, Watch eingebettet, Geh-Modus Watch, iPhone Geh + Edit mit Cross-Dept-Drag. iPhone-Einkauf: kein Navigationstitel „Einkaufsliste“, Toolbar kompakt.
...
- [ ] Settings-Reihenfolge (iPhone-Tab **Einstellungen**, `gearshape`): **Allgemein** (Hell/Dunkel/System, Creme/Blau) → **Einkauf** (Backup import/export/teilen, Archiv… / Archiv teilen, Inbox) → Aktueller Laden → Neuer Laden → Ladenweg (`sonstiges` movable, `vor`/`nach` locked) → **Eigene Abteilungen** (anlegen / umbenennen / löschen mit Confirm; Löschen remappt nach `sonstiges` und entfernt aus allen Layouts) → Abteilungen hinzufügen (inkl. Custom-IDs) → Reset → Stamm → Gespeicherte Listen (Bearbeiten + Übernehmen + Swipe-Delete) → Wörterbuch → **To-Do** (JSON-Backup / To-Do Backup import/export/teilen + Archiv… / Archiv teilen; Einkauf-JSON abgelehnt).
- [ ] `ListGrouping` sanitized inkl. `sonstiges`-Position und Custom-IDs; Rest-Depts mit Items nach dem letzten Nicht-`nach`-Gang; kein Remap von bekannter `item.dept` nach `sonstiges`.
- [ ] Eigene Abteilungen: `customDepartments` im Backup-Roundtrip; alte Backups `[]`; Watch zeigt Catalog-Titel; Löschen → Items/Staples/SavedLists/`mappings` nach `sonstiges`. Kein HTML-UI.
...
- [ ] Gespeicherte Listen: Name+Dept-Snapshot, füllen nicht ersetzen, Backup-Feld `savedLists`. iPhone-Einstellungen: Umbenennen + Inhalt bearbeiten (`SavedListEditView`), **Übernehmen** separat von Edit, Swipe-Delete mit Confirm. Overflow-Menü bleibt Tippen = Apply.
- [ ] Backup `einkauf-backup` mit stores, items (`imported`/`urgency` mitgeschrieben), staples, savedLists, `customDepartments`, walkMode, …; Backup teilen; Liste teilen PDF mit leeren Quadrat-Kästchen, respektiert das iPhone-Auge (`einkauf.iphone.hideCompleted`). Roundtrip behält `imported`, `urgency` und eigene Abteilungen. Alte Backups ohne `customDepartments` → `[]`.
- [ ] Einkauf-Artikel: `imported` nur Inbox-Abruf / expliziter Fremd-Datei-Anhang (nie Siri/Tippen/Stamm/Saved-List; Backup-Restore und eigenes PDF stempeln nicht); UI-Streifen teal/`theme.slate`, read-only, **ganz rechts** nach dem Urgency-Chip (links nur Checkbox + Text). `urgency` Chip ⚡ / ↔ / ↓ (nicht leerer Rahmen, nicht `<->`, nicht ○, nicht –), Zyklus eilig→normal→später, iPhone tippbar, Watch nur Anzeige. Nicht auf To-Do.
...
- [ ] Watch-Complication (WidgetKit, watchOS 10): nur offene Anzahl (`compactCountText`, bei 0 „erledigt“), Titel **Einkauf** wo Platz (Rectangular / Corner / Inline; Circular nur Zähler), Tap öffnet Geh-Modus; Update aus `einkauf-local.json` / WatchConnectivity; nicht auf dem iPhone.
- [ ] iPhone-Widget (WidgetKit, iOS 17): `systemSmall` zwei gestapelte Blöcke — Label in eigener Zeile (Caption, `lineLimit(1)`), darunter `oo/xx/yy` (Headline rounded semibold, monospaced, volle Breite); Abstand zwischen den Blöcken; `Einkaufsliste` sonst **Einkauf**; `To Do (<Liste>)` (`To Do (Alle)` bei leerer ID); `systemMedium`/`systemLarge` Mini-Tabelle Offen / Erledigt / Gesamt unverändert; To-Do nur aktuelle Liste; Taps `einkauf://list` / `einkauf://todo`; App Group `group.net.tschelle.einkauf`; nicht auf der Watch.
...
- [ ] `TabView` **Einkauf | To-Do | Einstellungen** auf dem iPhone (`gearshape`); Watch nur **Einkauf | To-Do**; getrennte Stores/Dateien/Backups (`todo-local.json` / `kind: "todo-local"` / `format: "todo-v3-json"`); Archive `einkauf-archiv.json` / `todo-archiv.json` (Append nur beim Löschen erledigter Einträge); WC-Context `{einkauf, todo}`; nie To-Do in `ShoppingStore` / `einkauf-local.json`.
- [ ] Archiv teilen und **Archiv…** (neueste zuerst; **Zurückspielen** = offene Kopie auf die Live-Liste, Archiv bleibt; Dauerhaft löschen einzeln) nur im iPhone-Tab Einstellungen (Sektion **Einkauf** und **To-Do** / To-Do Backup), analog Backup-Share; kein Archiv im Listen-Overflow, kein Watch-Sync, **kein** „Archiv leeren“ / Clear-All.
- [ ] iPhone-To-Do: kein Navigationstitel, Toolbar kompakt (`einkaufToolbarChrome`); Text, Person, Prio A/B, Datum, optionales `listId`; Zeile zeigt `#uid` Badge + reopen-Pills wie HTML sowie **Abgeschlossen-Datum** wenn gesetzt; Auge `todo.iphone.showCompleted`; Listenfilter `todo.iphone.currentListId` (leer = **Alle**); **Edit** / **Fertig** (`TodoEditSheet`); Swipe-Löschen **nur** Edit (`todo-browse` / `todo-edit`); Wieder öffnen; Sort `todo.iphone.sortKey`; Suche **Person oder Text …**; Backup `todo-v3-json` inkl. `lists`; **Liste teilen** PDF `TodoListPDF` folgt Liste + Auge.
- [ ] Watch-To-Do nur Geh-Modus (Toggle, Auge `todo.watch.hideCompleted`, kompaktes `#uid`); Filter `todo.currentListId` (WC-Feld, leer = Alle); kein Reopen/Suche/Sort/Edit/Listen-UI. Complication `TodoProgress`: Label = aktueller Listenname (Fallback **Alle**), offene Anzahl der aktuellen Liste / **erledigt**, `todo-local.json` + `todo.currentListId`, Tap `einkauf://todo`. Circular nur Zahl.
