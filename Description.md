...
Stand der nativen App: 2026-09-03 (Build 28, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
| Geteilter Code | `Sources/Shared` in beiden App-Targets (`ConnectivitySync.swift` nur Apple-Targets, nicht im SPM-Paket; `SpeechItemSplitter.swift` + `addItems(fromSpeech:)` rein, Linux-testbar, mögliche spätere iPhone-Sprache — **nicht** auf der Watch). Widgets laden `einkauf-local.json` über `Persistence` (ohne `ShoppingStore` / WatchConnectivity im Widget-Prozess). |
...
| Xcode | 15+, iOS 17, watchOS 10, Sprache de, Marketing 1.0. Watch-Target linkt **kein** `Speech.framework` / `AVFoundation`; kein Bridging-Header, kein ObjC-Shim, kein `import Speech`. |
...
**Keine Sprache, kein Diktat, kein Mikrofon** (PRs #42–#48 zurückgenommen). Die Chrome-Zeile ist wieder **nur Auge** + `Spacer()` — kein `mic`, kein `WatchVoiceAdd`, kein `WatchDictatePanel`, kein `TextFieldLink`, kein `presentTextInputController`, kein Hold-to-Talk, kein `Speech.framework` / `AVFoundation`, kein ObjC-Shim, kein Bridging-Header. Watch-`Info.plist` ohne `NSMicrophoneUsageDescription` / `NSSpeechRecognitionUsageDescription`. `SpeechItemSplitter` und `ShoppingStore.addItems(fromSpeech:)` bleiben in `Sources/Shared` (Tests; mögliche spätere iPhone-Sprache) und splittet an Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `. Die Watch ruft sie nicht auf; Watch-only-APIs (`addItemsFromWatchVoice`, weiches Persist nur für Übernehmen) sind entfernt.
...
**Nur native / nicht ins HTML:** Watch (Geh-Modus, Titel gekürzter Laden + Einkauf xx/yy, Erledigte ausblendbar, **kein** Mikrofon/Diktat/Sprache, WidgetKit-Complication `xx/yy`, kein Picker/Edit/Share); iPhone-Geh-Modus mit eigenem Auge (Bearbeiten ungefiltert); **iPhone-Homescreen-Widget** (`systemSmall`/`systemMedium`, Tap öffnet Einkaufsliste); **PDF Liste teilen** mit leeren quadratischen Kästchen; Erscheinungsbild **System** (folgt iPhone-Appearance) plus Hell/Dunkel und Creme/Blau **nur in Einstellungen**, nicht in der Toolbar. TestFlight nicht erforderlich. Native-`guess` wertet Nutzer-Mappings vor Sonderregeln/Keywords aus (HTML-Reihenfolge kann abweichen).
...
- Watch bleibt Geh-Modus ohne Picker/Edit/Share/Sprache/Diktat/Mikrofon (Auge blendet Erledigte nur an, löscht sie nicht); iPhone-Auge nur Geh-Modus, Bearbeiten ungefiltert; Flags geräte-lokal, nicht im Backup
...
- [ ] Watch-Titel: gekürzter Ladenname + Einkauf xx/yy; Auge blendet Erledigte nur in der Watch-Gehliste aus (`einkauf.watch.hideCompleted`, nicht Backup); iPhone-Geh-Modus hat dasselbe Auge mit `einkauf.iphone.hideCompleted`; Bearbeiten ungefiltert; kein Picker/Edit/Share/Sprache/Diktat/Mikrofon auf der Watch; WatchConnectivity.
