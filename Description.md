...
Stand der nativen App: 2026-09-03 (Build 26, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
| Xcode | 15+, iOS 17, watchOS 10, Sprache de, Marketing 1.0. Watch-Spracheingabe **ohne** `Speech.framework` / `AVFoundation` / Bridging-Header: Mic öffnet ein **In-App-TextField**-Panel (Prompt „Artikel“, **Übernehmen** committet, X bricht ab) — **nicht** `TextFieldLink`, **nicht** `presentTextInputController`. Speech-Header sind in dieser watchOS-Toolchain nicht verfügbar (`Speech/Speech.h` file not found); kein ObjC-Shim, kein `import Speech`. |
...
**Mikro:** in derselben Chrome-Zeile **trailing** nach `Spacer()`. SF-Symbol `mic` (`mic.fill` / `theme.oxide` während das Diktier-Panel offen ist). `.font(.caption)`, `.imageScale(.small)`, `.buttonStyle(.plain)` — **kein** `.title2` / `.title`, **kein** Toolbar-Item (sonst der große gefüllte Kreis). **Tippen** setzt `showDictate` und öffnet ein **In-App-TextField**-Panel (restliche Fläche unter Chrome/Titel, kein System-Modal): Prompt „Artikel“ (watchOS-`TextField` hat Scribble/Diktat über die Systemsteuerung). Primärknopf **Übernehmen** committet und schließt; **X** / Abbrechen schließt ohne Commit. Commit erst nach **Übernehmen** auf unserer UI — nicht während Keyboard-/Diktat-Dismiss, **nicht** `onSubmit`/`TextFieldLink`, **nicht** `presentTextInputController` auf einem `WKInterfaceController` (Crash-on-Dismiss in reiner SwiftUI-Watch-App), **nicht** Hold-to-Talk, nicht gedrückt halten, kein Dauer-Zuhören, kein `Speech.framework`. Speech-Header sind in dieser watchOS-Toolchain nicht verfügbar (`Speech/Speech.h` file not found); kein ObjC-Shim, kein Bridging-Header, kein `import Speech`, kein `SWIFT_ENABLE_EXPLICIT_MODULES`-Workaround. Kein `NSMicrophoneUsageDescription` / `NSSpeechRecognitionUsageDescription` — In-App-`TextField` braucht diese Keys nicht. Entitlements bleiben App Group. Abbruch: kein Crash, kein Persist. Leerer oder unbrauchbarer Text: Status „Nichts verstanden.“ Treffer: `SpeechItemSplitter` teilt an Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; Trim, Leere weg, Whitespace zusammenziehen; Mengenwörter bleiben im Namen („zwei Eier“), damit `DepartmentGuesser` / mappings greifen. `addItems(fromSpeech:)` legt alle Stücke in **einem** State-Update an und ruft **einmal** `persistAndSync()` (nicht N× `addItem`). Sync WatchConnectivity. Kurzes Haptic wenn Artikel dazukamen; Status in der Listen-Chrome nach dem Schließen. **iPhone-Spracheingabe kommt später** (derselbe Splitter in `Sources/Shared`).
...
- [ ] Watch-Mikro: Tippen öffnet In-App-TextField-Panel, Übernehmen splittet/`addItems(fromSpeech:)` einmal persist; kein Speech-Framework, kein TextFieldLink/presentTextInput; iPhone-Sprache später.
