...
Stand der nativen App: 2026-09-03 (Build 22, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
| Watch-UI | `Sources/Watch` — `EinkaufWatchApp`, `WatchListView`, `WatchHoldToTalk`, `WatchComplicationReload` |
...
| Xcode | 15+, iOS 17, watchOS 10, Sprache de, Marketing 1.0. Target **EinkaufWatch** linkt `Speech.framework` + `AVFoundation.framework` (`OTHER_LDFLAGS` `$(inherited) -framework Speech -framework AVFoundation`); `SWIFT_ENABLE_EXPLICIT_MODULES = NO` nur dort (expliziter Modul-Scanner findet sonst `import Speech` nicht). |
...
**Hold-Mikro:** in derselben Chrome-Zeile **trailing** nach `Spacer()`. SF-Symbol `mic` / `mic.fill` während der Aufnahme. `.font(.caption)`, `.imageScale(.small)`, `.buttonStyle(.plain)` — **kein** `.title2` / `.title`, **kein** Toolbar-Item (sonst der große gefüllte Kreis). Sprache **nur solange gedrückt** (press → Start, release → Stop und Commit). Kein Dauer-Zuhören. Erste Nutzung: System fragt Mikrofon und Spracherkennung; Watch-`Info.plist` `NSMicrophoneUsageDescription` und `NSSpeechRecognitionUsageDescription` (Deutsch, Zweck: Einkaufsliste per Sprache füllen). Entitlements bleiben App Group, kein extra Speech-Entitlement. Target `EinkaufWatch` linkt `Speech.framework` und `AVFoundation.framework`; `SWIFT_ENABLE_EXPLICIT_MODULES = NO` (sonst `Unable to find module dependency: 'Speech'`). On-device `SFSpeechRecognizer` + Audio-Engine, Locale `de_DE`, sonst Geräte-Locale. Permission beim ersten Druck. Release ohne nutzbaren Text: Erkennung abbrechen, Status „Nichts verstanden.“; verweigert: „Mikrofon nicht erlaubt.“ / „Spracherkennung nicht erlaubt.“ — kein Crash. Treffer: `SpeechItemSplitter` teilt an Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; Trim, Leere weg, Whitespace zusammenziehen; Mengenwörter bleiben im Namen („zwei Eier“), damit `DepartmentGuesser` / mappings greifen. Jedes Stück `addItem` über `addItems(fromSpeech:)` , Sync WatchConnectivity. Kurzes Haptic wenn Artikel dazukamen. **iPhone-Spracheingabe kommt später** (derselbe Splitter in `Sources/Shared`).
