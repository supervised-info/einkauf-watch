...
Stand der nativen App: 2026-09-03 (Build 21, `CURRENT_PROJECT_VERSION`). Nur **diese eine** Spec-Datei im Repo-Root (`Description.md`, kein zweites `Description_index.md`). Swift-Quellen sind die Wahrheit: bei Widerspruch den Code prüfen, nichts erfinden, die Website nicht scrapen.
...
**Hold-Mikro:** in derselben Chrome-Zeile **trailing** nach `Spacer()`. SF-Symbol `mic` / `mic.fill` während der Aufnahme. `.font(.caption)`, `.imageScale(.small)`, `.buttonStyle(.plain)` — **kein** `.title2` / `.title`, **kein** Toolbar-Item (sonst der große gefüllte Kreis). Sprache **nur solange gedrückt** (press → Start, release → Stop und Commit). Kein Dauer-Zuhören. Erste Nutzung: System fragt Mikrofon und Spracherkennung; Watch-`Info.plist` `NSMicrophoneUsageDescription` und `NSSpeechRecognitionUsageDescription` (Deutsch, Zweck: Einkaufsliste per Sprache füllen). Entitlements bleiben App Group, kein extra Speech-Entitlement. On-device `SFSpeechRecognizer` + Audio-Engine, Locale `de_DE`, sonst Geräte-Locale. Permission beim ersten Druck. Release ohne nutzbaren Text: Erkennung abbrechen, Status „Nichts verstanden.“; verweigert: „Mikrofon nicht erlaubt.“ / „Spracherkennung nicht erlaubt.“ — kein Crash. Treffer: `SpeechItemSplitter` teilt an Komma, Semikolon, Zeilenumbruch, ` und `, ` sowie `; Trim, Leere weg, Whitespace zusammenziehen; Mengenwörter bleiben im Namen („zwei Eier“), damit `DepartmentGuesser` / mappings greifen. Jedes Stück `addItem` über `addItems(fromSpeech:)` , Sync WatchConnectivity. Kurzes Haptic wenn Artikel dazukamen. **iPhone-Spracheingabe kommt später** (derselbe Splitter in `Sources/Shared`).
