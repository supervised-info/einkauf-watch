...
/// Watch-Siri legt nur gesprochene Aufgaben-Strings in die App-Group-Queue.
/// Die Watch-App drain't sie auf dem live `TodoStore` — kein `TodoStore` / kein Encode im Intent.
/// Primär: `UserDefaults(suiteName: Persistence.appGroupId)`, Key `todo.siriPendingAdds`.
/// Datei `todo-siri-pending.json` nur Spiegel im App-Group-Ordner `Einkauf/`.
/// Getrennt von der Einkaufs-Siri-Queue.
enum TodoSiriPendingAdds {
    static let fileName = "todo-siri-pending.json"
    static let defaultsKey = "todo.siriPendingAdds"
    ...
    static func enqueue(_ speech: String, defaults: UserDefaults? = nil, at url: URL? = nil) {
        let text = SpeechItemSplitter.strippingTodoTriggerPrefix(speech)
