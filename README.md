...
**iPhone To-Do:** Text, Person, Prio A/B, Datum, benannte Listen (`todo.iphone.currentListId`, leer = **Alle**). Auge blendet Abgeschlossene (`todo.iphone.showCompleted`). Toolbar **Edit** / **Fertig**: Swipe-Löschen **nur** im Edit-Modus; Listen-Modus öffnet `TodoEditSheet` per Text oder Swipe **Edit**. Wieder öffnen, Sort, Suche (**Person oder Text …**), eigenes Backup `todo-v3-json` inkl. `lists`, MD/CSV (volle Liste), **Liste teilen** (PDF folgt Liste + Auge).
